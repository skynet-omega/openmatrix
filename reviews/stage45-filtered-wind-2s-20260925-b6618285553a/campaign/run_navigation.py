"""Prospective one-second continuation of one fixed-source full-organism arm."""
...
CAPTURE_EPOCHS = ('preparacion:1','preparacion:40','ensayo:1','ensayo:100',
                  'ensayo:400','ensayo:1000','ensayo:1500','ensayo:2000')

def source_files() -> tuple[Path,...]:
    return (HERE/'PLAN.json',HERE/'run_navigation.py',HERE/'motor_wind.py',
            HERE/'CAMPOS.json',HERE/'reference/ORN_INDEX_MAP.json',
            HERE/'preflight_reference_guard.py',HERE/'REFERENCE_POSE_GUARD_01.json',
...
def require_sources() -> dict:
    ...
    if (plan['fixed_inputs']['local_pose_guarded_helper_sha256']!=frozen[str((GAUSSIAN/'gaussiano_400.py').resolve())]
            ...
            or plan['fixed_inputs']['reference_pose_guard_sha256']!=frozen[str((HERE/'REFERENCE_POSE_GUARD_01.json').resolve())]
...
def main() -> int:
    ...
    obj = session = flow_tap = drive_observer = undo = auditor = motor_wind = None
    rows = []
    status = 'STARTING'
    error = None
    cleanup = []
    ...
    try:
        ...
        atomic_json(a.out/'RUN_CONTRACT.json',{
            'schema':'stage45_two_second_filtered_wind_screen_v1','field':a.field,'engine':a.engine,
            ...
            'biological_parameters_changed':False,'interface_intervention':'PN629 disabled before preparation; fixed Gaussian source, causal motor threshold/filter and physical wind pulse during trial',
            'source_mode':'The selected mirror source remains world-fixed for 2000 ms; this is an exploratory trajectory screen',
            ...
            'motor_policy':{'kind':'causal_200ms_ema_with_symmetric_threshold_and_saturated_5deg_s_output',
                            'tau_s':MotorWind.TAU_S,'threshold_rad_s':MotorWind.THRESHOLD_RAD_S,
                            'output_rad_s':MotorWind.OUTPUT_RAD_S,
                            'wind_first_step':MotorWind.WIND_FIRST_STEP,
                            'wind_last_step':MotorWind.WIND_LAST_STEP,
                            'wind_torque_native':MotorWind.WIND_TORQUE_NATIVE}})
        ...
        for phase, duration in [('preparacion',prep),('ensayo',trial_ms)]:
            ...
            for k in range(duration):
                ...
                row['neural_command_raw_rad_s']=float(row['command_yaw_rate_rad_s'])
                row['motor_filter_applied_rad_s']=float(row['command_yaw_rate_rad_s'])
                row['motor_filter_state_rad_s']=0.0
                row['wind_torque_native']=0.0
                if phase == 'ensayo':
                    if motor_wind.trial_step != k+1:
                        raise RuntimeError('Motor filter did not advance once per CNS interval')
                    row['neural_command_raw_rad_s']=motor_wind.raw
                    row['motor_filter_applied_rad_s']=motor_wind.applied
                    row['motor_filter_state_rad_s']=motor_wind.filtered
                    row['wind_torque_native']=MotorWind.WIND_TORQUE_NATIVE if (
                        MotorWind.WIND_FIRST_STEP<=k+1<=MotorWind.WIND_LAST_STEP) else 0.0
                ...
                if phase == 'ensayo' and k+1 in (1000,1020,1500):
                    storage.snapshot(f'state_{k+1}ms',snapshot)
                if (k+1)%10 == 0:
        ...
        if motor_wind is not None:
            try:
                motor_receipt=motor_wind.audit()
                atomic_json(a.out/'MOTOR_WIND_AUDIT.json',motor_receipt)
                if status=='COMPLETE' and motor_receipt['wind_substeps']!=motor_receipt['expected_wind_substeps']:
                    cleanup.append('wind step count mismatch');status='INCOMPLETE'
            except BaseException as exc:
                cleanup.append('motor/wind audit: '+repr(exc));status='INCOMPLETE'
        try:
            save_trace()
        except BaseException as exc:
        ...
        atomic_json(a.out/'RESULT.json',{'status':status,'error':error,'cleanup_errors':cleanup,
            ...
            'reason':'Single two-second filtered motor and physical wind trajectory; exploratory screen. An unfiltered matched arm and perturbation control are required for Stage4/5 causal claims.'})

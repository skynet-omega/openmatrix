...
class FixedGaussian:
    ...
    def sample(self,data,source_mm,sigma_mm):
        ...
        need(0<=elapsed<=2000*DT and elapsed%DT==0,'Reloj fuera del piloto')
...
class Intervals:
    ...
    def before(self,obj):
        need(not self.invalid and not self.armed and self.n<2000,'Auditor inválido/reentrante')
    ...
    def after(self,obj,row):
        try:
            ...
            need(np.isfinite(command) and np.isfinite(row['command_yaw_rate_rad_s']),
                 'Mando neural o aplicado no finito')
            need(abs(row['command_yaw_rate_rad_s'])<=np.deg2rad(5.)+1e-15,
                 'Mando aplicado fuera del límite')
            need(command==row['neural_command_raw_rad_s'],
                 'Lector neural previo al filtro alterado')
            need(row['command_yaw_rate_rad_s']==row['motor_filter_applied_rad_s'],
                 'Mando filtrado no llegó al cuerpo')
            r={'sample_used_ns':end-DT,'sample_pending_ns':end,'used':self.used.tolist(),
               ...
               'neural_command_raw_rad_s':float(command),
               'command_yaw_rate_rad_s':float(row['command_yaw_rate_rad_s'])}
    ...
    def export(self,path):
        save(path,{'field':self.field.metadata(),'prepared_pose_guard':self.prepared_pose_guard,'intervals':self.records,
                   'complete':self.n==2000 and not self.invalid and not self.armed,

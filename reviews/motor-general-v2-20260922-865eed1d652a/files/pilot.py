...
def exact(name,t):
    ...
    return None
def main():
    p=argparse.ArgumentParser();p.add_argument('out');p.add_argument('--case',required=True);p.add_argument('--profile',choices=['fast','precise']);p.add_argument('--reference',action='store_true');p.add_argument('--reference-dir');a=p.parse_args()
    ...
    ctx={'case':a.case,'model_identity':m.identity,'profile':a.profile,'end':end,'states':m.n,'connections':m.connection.nnz,'mass_nnz':m.mass.nnz,'times':times.tolist()}
    ...
    if a.reference:
        ...
        e=Implicit(spec,a.profile);ys=[e.read()];start=time.perf_counter();failure=None
        try:

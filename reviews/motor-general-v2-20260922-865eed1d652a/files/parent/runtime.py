...
class Engine:
    def __init__(self,spec,backend='gpu',profile='precise',algorithm='A'):
        ...
        self.gpu=GPU(self.model,profile,algorithm) if backend=='gpu' else None
    ...
    def advance(self,end,wall_limit_s=300):
        ...
        if self.gpu:
            ...
            rtol,atol=PROFILES[self.profile]
            def rhs(t,y):
    ...
    def checkpoint(self,path):
        ...
        meta={'version':1,'spec':self.model.spec,'model_sha256':self.model.identity,'backend':self.backend,'profile':self.profile,'algorithm':self.algorithm,'time':self.t,'next_h':self.next_h,'edits':self.edits,'cpu_method':self.cpu_method,'state_sha256':__import__('hashlib').sha256((path/'state.npy').read_bytes()).hexdigest(),'rng':'absent: deterministic domain','event_queue':'absent: only external edits at explicit advance boundaries'}
    ...
    def restore(cls,path):
        ...
        meta=json.loads(raw)
        require(meta['version']==1,'checkpoint schema')
        require(__import__('hashlib').sha256((path/'state.npy').read_bytes()).hexdigest()==meta['state_sha256'],'checkpoint state corruption')
        require(digest(meta['spec'])==meta['model_sha256'],'checkpoint model corruption')
        e=cls(meta['spec'],meta['backend'],meta['profile'],meta['algorithm']);y=np.load(path/'state.npy',allow_pickle=False)
        require(y.shape==(e.model.n,) and np.isfinite(y).all(),'invalid checkpoint state')

"""Coupled, stiffly accurate two-stage SDIRK2 on the unchanged fine PN.

C*v' = I - G*v - Iion(v,x), x' = (x_inf(v)-x)/tau(v).
The gate equation of each implicit stage is eliminated exactly at trial v.
Newton uses its full local derivative. Signed derivatives are linearization
terms only; G, C, channel kinetics, placement and gbar remain unchanged.
External current is constant throughout each call; split at discontinuities.
Optional synaptic conductances are presampled at the two implicit stage times.
All stages, charge and clock commit together. No clipping, reset or holding.
"""
import hashlib
import inspect
import numpy as np
from kc_four_port_active import channel_rates
from pn_fine_ionic import FinePNIonicSession,channel_conductance

GAMMA=1.-1./np.sqrt(2.)
METHOD='coupled_SDIRK2_gamma_1_minus_inv_sqrt2_v1'


def stage_channels(voltage_mV, gate_base, duration_s, gbar_nS, reversal_mV):
    """Implicit gate stage, currents and exact local dI/dV including gates."""
    v=np.asarray(voltage_mV);base=np.asarray(gate_base)
    with np.errstate(over='ignore',invalid='ignore',divide='ignore'):
        steady,tau=channel_rates(v)
        x=(tau*base+duration_s*steady)/(tau+duration_s)
        residual=x-base-duration_s*(steady-x)/tau
        ds=steady*(1-steady)*np.array([.1121,-.2,.2717,.0502])
        sm=(tau[:,0]*1000-.1270)/3.434;sn=(tau[:,3]*1000-2.03)/1.96
        dtau=np.column_stack((-3.434/5.98*sm*(1-sm),
            -(tau[:,1]*1000-.36)/10.47,np.zeros(len(v)),-1.96/3.12*sn*(1-sn)))/1000
        dx=(duration_s*ds+dtau*(base-x))/(tau+duration_s)
        m,h,p,n=x.T;dm,dh,dp,dn=dx.T
        g=channel_conductance(x,gbar_nS)
        dg=gbar_nS*np.column_stack((3*m*m*h*dm+m**3*dh,dp,4*n**3*dn))
        ionic=g*(v[:,None]-reversal_mV)
        jac=(g+dg*(v[:,None]-reversal_mV)).sum(axis=1)
    if (not np.isfinite(x).all() or np.any(x<0) or np.any(x>1)
            or not np.isfinite(tau).all() or np.any(tau<=0)
            or not np.isfinite(jac).all() or not np.isfinite(ionic).all()):
        raise FloatingPointError('Invalid coupled stage kinetics; reject without clipping')
    return x,ionic,jac,float(np.max(abs(residual)))


class CoupledFinePNIonicSession(FinePNIonicSession):
    def __init__(self,*args,**kwargs):
        super().__init__(*args,**kwargs)
        self.first_order_identity=self.identity
        self.identity=hashlib.sha256((self.identity+METHOD+inspect.getsource(stage_channels)).encode()).hexdigest()

    def import_first_order_state(self,state):
        """Explicit method migration; preserve all physical state and its clock."""
        if self.time_ns!=0 or np.any(self.ionic_charge_pC) or bool(self.cp.any(self.voltage)):
            raise ValueError('Import requires a fresh target session')
        if state.get('identity')!=self.first_order_identity:
            raise ValueError('First-order parent does not match anatomy and channels')
        self.load_state_dict(dict(state,identity=self.identity))
        return dict(source_identity=self.first_order_identity,target_identity=self.identity,
                    time_ns=self.time_ns,physical_state_preserved=True,method=METHOD)

    def advance(self,dt_ns,current_pA,*,rtol=1e-9,atol=2e-12,maxiter=220,
        ...
        def stage(vbase,xbase,guess,synapse,ca_base,ca_update,threshold_guess=None):
            ...
            def evaluate(w):
                x,ionic,jac,gate_error=stage_channels(cp.asnumpy(w[nodes])+self.leak_reversal_mV,
                    xbase,q,self.gbar_nS,self.reversal_mV)
                residual=C*((w-vbase)*shift)+self.backend.action(w)-current
                ...
                cdata=None
                if ca is not None:
                    cdata=ca.stage(cp.asnumpy(w[cn])+self.leak_reversal_mV,ca_base,q)

...
def main():
 ...
 if a.ms not in (100,1000):raise ValueError('Only original horizon or prospectively narrowed historical window allowed')
 if a.ms==100 and not (HERE/'AMENDMENT_01.json').is_file():raise ValueError('Missing prospective scope amendment')
 ...
 try:
  ...
  for file in HERE.glob('*'):
   if file.suffix in ('.py','.cpp') or file.name in ('PLAN.json','AMENDMENT_01.json'):shutil.copy2(file,frozen/file.name)

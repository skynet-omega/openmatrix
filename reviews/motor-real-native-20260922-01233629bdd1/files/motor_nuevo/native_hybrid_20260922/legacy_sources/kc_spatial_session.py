"""Persist a complete CNS/body with an explicit spatial WT9 KCgamma candidate."""
from pathlib import Path
import copy,json,shutil,tempfile
import numba
import numpy as np
from kc_electrical_scales_session import (
    KcElectricalScalesSession,SOURCES as PARENT_SOURCES,_digest,_fingerprints,
...
from kc_spatial_brain import GpuKcSpatialBrain

SCHEMA='matrix_kc_spatial_session_v1'
SOURCES=tuple(PARENT_SOURCES)+('kc_four_port_active.py','kc_projected_active.py','kc_projected_batch.py','kc_spatial_inputs.py','kc_spatial_brain.py','kc_spatial_session.py')


class KcSpatialSession(KcElectricalScalesSession):
    @classmethod
    def from_checkpoint(cls,path,*,enabled=True,pn_enabled=True,apl_enabled=True,inner_step_ns=25000):
        path=Path(path).resolve();parent=KcElectricalScalesSession.load(path)
        ...
        try:
            ...
            obj.hybrid=GpuKcSpatialBrain.adopt(old,enabled=enabled,pn_enabled=pn_enabled,apl_enabled=apl_enabled,inner_step_ns=inner_step_ns);m=obj.hybrid.kc_spatial_manifest
            obj.config=copy.deepcopy(obj.config)
            obj.config.update(candidate='KC_SPATIAL_WT9_v1',kc_spatial_enabled=enabled,
            ...
            new=obj.hybrid.state_dict();new.pop('schema');new.pop('kc_spatial_manifest');new.pop('kc_spatial_state')
            ...
            obj.intervention=dict(previous_intervention=copy.deepcopy(obj.intervention),
                operation='Connect canonical PN/APL conductances to17-port WT9 KCgamma references in the continuing full CNS',
                ...
                new_spatial_voltage_states=17*len(m['inputs']['rows']),pn_enabled=pn_enabled,apl_enabled=apl_enabled,inner_step_ns=inner_step_ns,APL_absolute_conductance_calibrated=False,
    ...
    def _validate(self):
        super()._validate()
        if not isinstance(self.hybrid,GpuKcSpatialBrain):raise ValueError('Missing projection conductance component')
        self.hybrid.validate_spatial()
        if self.config['kc_spatial_enabled']!=self.hybrid.kc_spatial_manifest['enabled']:
            raise ValueError('Spatial KC configuration disagrees')

    def state_dict(self):
        result=super().state_dict();result['schema']=SCHEMA;return result

    def _manifest_fields(self):
        result=super()._manifest_fields();m=self.hybrid.kc_spatial_manifest
        result.update(schema=SCHEMA,kc_spatial_enabled=m['enabled'],
            spatial_KCgamma_cells=len(m['inputs']['rows']),spatial_voltage_ports=17,
            spatial_PN_enabled=m['pn_enabled'],spatial_APL_enabled=m['apl_enabled'],
            spatial_inner_step_ns=m['inner_step_ns'],spatial_source_artifact=m['cell']['artifact_sha256'],
            physiological_status='Explicit WT9 alternative template with inherited conductance scales; no individual physiology identified')
        return result
    ...
    def save(self, path):
        ...
        if self.source_identity != {name: sha256(ROOT/'src'/name) for name in SOURCES}:
            raise ValueError('Runtime source changed after spatial KC session creation')
    ...
    def load(cls, path):
        ...
        if manifest.get('schema') != SCHEMA:
            raise ValueError('Unsupported spatial KC checkpoint')
        actual = {str(p.relative_to(path)) for p in path.rglob('*') if p.is_file()}
        if actual != set(manifest['files']) | {'manifest.json'}:
            raise ValueError('Incomplete spatial KC checkpoint')
        ...
        if set(state) != SCALARS | CYBORG_SCALARS | special or state['schema'] != SCHEMA:
            raise ValueError('Unsupported or incomplete spatial KC state')
        ...
        kinds = {kind.SCHEMA: kind for kind in (GpuKcSpatialBrain,)}
        kind = kinds.get(state['hybrid']['schema'])
        if kind is None:
            raise ValueError('Unknown spatial KC neural component schema')
        ...
        try:
            ...
            if any(manifest.get(k) != v for k, v in obj._manifest_fields().items()):
                raise ValueError('Manifest and spatial KC neural/body state disagree')
            return obj
        except BaseException:
            obj.body.close()
            raise

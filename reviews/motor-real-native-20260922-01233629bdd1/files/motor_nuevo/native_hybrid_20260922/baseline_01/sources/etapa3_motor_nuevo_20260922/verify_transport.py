...
from compare import compare,read_state,flatten
...
def history_difference(a,b):
 def validate(x):
  ...
  for i,s in enumerate(h):
   ...
   if s['left'].shape!=s['right'].shape or not np.isfinite(s['left']).all() or not np.isfinite(s['right']).all():raise ValueError('Invalid delay driver')
...
def verify(reference,candidate):
 ...
 passed=r['screen_pass'] and not unexpected and not changed_flags and all(x['max_abs']<=1e-4 for x in history.values())
 return {'reference':str(reference),'candidate':str(candidate),'metrics':r['metrics'],'limits':r['limits'],'exact_all_leaves':r['exact_all_leaves'],'histories':history,'history_normalized_limit':1e-4,'unexpected_layouts':unexpected,'changed_flags':changed_flags,'screen_pass':bool(passed),'scope':'Short numerical transport screen only. History criterion extends coverage; no long-time certification.'}

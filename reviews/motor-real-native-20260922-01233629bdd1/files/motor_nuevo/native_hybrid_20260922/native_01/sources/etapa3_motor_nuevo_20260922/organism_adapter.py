...
class OrganismAdapter:
 def __init__(self,brain,events):
 ...
 def build(self,drive,light):
  ...
  def coefficient(z):
   ...
   return a,r
  try:
   self.core=NativeGraph(b.state,coefficient,rtol=b.parameters['rtol'],atol=b.parameters['atol'],norm_size=b._norm_size(),project=lambda z,c,f:self.ports.project(z,c,f),freeze=freeze)
  finally:
 ...
 def step(self,b,ns,drive,light):
  ...
  with g.stream:
   ...
   self.ports.update(self.events.active)
  p=b.parameters
  nxt,counts,error=g.advance(ns,b.next_step_ns,p['minimum_step_ns'],p['maximum_step_ns'])
...
def install(brain,events):
 adapter=OrganismAdapter(brain,events);old=events.step;events.step=adapter.step

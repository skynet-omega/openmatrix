...
class Cell:
 def __init__(self,b,events,compressed=False):
  if compressed:raise ValueError('Compressed operator not yet qualified')
  ...
  with cp.cuda.using_allocator(self.pool.malloc),self.stream:
   ...
   self.step_kernel=build_step();mod=cp.RawModule(code=(HERE/'physical_events.cu').read_text(),options=('--fmad=false',));self.somatic=mod.get_function('commit_event');self.axonal=mod.get_function('commit_axon')
 ...
 def step(self,v,g,split):
  ...
  self.step_kernel((b.n,),(32,),(np.int32(b.n),self.clock,np.int32(split),np.float64(b.rest),v,g,self.ge,self.gi,self.current,b.C,b.G,b.chanG,b.chanb,b.shuntG,b.shuntb,b.ena,out,gates,errors))

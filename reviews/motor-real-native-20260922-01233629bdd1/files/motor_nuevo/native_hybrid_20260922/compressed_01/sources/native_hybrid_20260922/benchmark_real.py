...
def main():
 ...
 p=argparse.ArgumentParser();p.add_argument('--out',type=Path,required=True);p.add_argument('--ms',type=int,default=5);p.add_argument('--native-cell',action='store_true');p.add_argument('--compressed',action='store_true');a=p.parse_args()
 import cupy as cp
 ...
 try:
  ...
  if a.native_cell:
   ...
   cell,undo=native_cell.install(h,events,compressed=a.compressed);restores.append(undo)

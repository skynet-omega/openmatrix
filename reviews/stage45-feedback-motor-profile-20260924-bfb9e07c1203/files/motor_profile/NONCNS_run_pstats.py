"""Frozen two-ms host-callgraph diagnostic; model/runtime are original sources."""
...
ROUND = DIAGNOSTIC.parent
HERE = ROUND.parent/'epoch_cost_20260923'
ROOT = HERE.parents[1]
OLD = Path('/home/daroch/AXIOMA_FLYWIRE/matrix')
sys.path[:0] = [str(OLD/'work/motor14_20260922'), str(OLD/'work/motor13_20260922'),
                str(ROOT/'motor_nuevo/pipeline_review_20260922'), str(HERE)]
from motor_runtime import load
from run_pipeline import RunStorage, atomic_json
from native_tap import NativeFlowTap

ORIGINAL_RUNNER_SHA256 = '4beef46a3149ea169712db67cf30168f7d258ba872af29ebaace07029ee9a95d'
DIAGNOSTIC_PLAN_SHA256 = '149125f07e0e352a750e7c9a3f20984f013b6e48668dd4d7902a23028a16a317'
EXTRACTOR_SHA256 = 'bc6aa3ecb8d42e08a557a7aa0d752e21fb20cb85c7875a0059b49f3c43463b1b'
CHECKPOINT_MANIFEST_SHA256 = '1817df128eb1f79953a6e45a24863465b6fe6c59b71fa0ddd6af4edde541ba15'
...
def main() -> int:
    ...
    diagnostic_plan = ROUND/'NONCNS_PSTATS_PLAN_01.json'
    extractor = ROUND/'NONCNS_pstats_arcs.py'
    if (_sha(HERE/'run_set.py') != ORIGINAL_RUNNER_SHA256 or
            _sha(diagnostic_plan) != DIAGNOSTIC_PLAN_SHA256 or
            _sha(extractor) != EXTRACTOR_SHA256):
        raise RuntimeError('Diagnostic runner, original runner or extractor source lock changed')
    if _sha(OLD/'work/stage234_settling_extension_20260915/settled_700ms/manifest.json') != CHECKPOINT_MANIFEST_SHA256:
        raise RuntimeError('Frozen checkpoint manifest changed')
    ...
    a = p.parse_args()
    if (a.odor != 'sham' or a.engine != 'causal_cuda' or a.ms != 2 or
            a.observe != 'off' or a.cuda_profile != 'off' or a.kc_capture != 'off'):
        raise ValueError('NONCNS plan permits only 2-ms sham causal_cuda without observer/KC capture/CUDA profiler')
    ...
    try:
        ...
        if plan['checkpoint'] != 'work/stage234_settling_extension_20260915/settled_700ms':
            raise RuntimeError('Loader selected a different checkpoint than the NONCNS plan')
        h = obj.core.hybrid
        ...
        frozen.mkdir()
        files = [DIAGNOSTIC,diagnostic_plan,extractor,ROUND/'NONCNS_runner_sanity.py',
                 HERE/'PLAN.json',HERE/'run_set.py',HERE/'native_tap.py',
        ...
        atomic_json(a.out/'NONCNS_SOURCE_HASHES.json',
                    {str(source.resolve()):_sha(source) for source in files})
        atomic_json(a.out/'RUN_CONTRACT.json',{
            ...
            'noncns_diagnostic_plan_sha256':DIAGNOSTIC_PLAN_SHA256,
            'noncns_original_runner_sha256':ORIGINAL_RUNNER_SHA256,
            'noncns_diagnostic_runner_sha256':_sha(DIAGNOSTIC),
            'noncns_extractor_sha256':EXTRACTOR_SHA256,
            'noncns_raw_profile':a.profile=='on',
            'noncns_profile_scope':'Only second ensayo obj.step; first ms unprofiled',
            'noncns_attribution_limit':'pstats caller functions, no per-callsite bytes'})
        ...
        if a.profile == 'on':
            ...
            raw=a.out/'NONCNS_RAW.pstats'
            profiler.dump_stats(str(raw))
            if raw.stat().st_size > 16*1024*1024:
                raise RuntimeError('Raw pstats exceeds frozen 16-MiB diagnostic cap')
            from NONCNS_pstats_arcs import extract
            arcs=extract(raw)
            target={r['kind']:r for r in arcs['rows'] if r['kind'] in ('cupy.ndarray.get','numpy.ndarray.tolist')}
            if set(target)!= {'cupy.ndarray.get','numpy.ndarray.tolist'} or any(
                    r['calls']<1 or r['arc_call_sum']!=r['calls'] for r in target.values()):
                raise RuntimeError('Raw profiler lacks complete get/tolist caller arcs')
            arc_data=json.dumps(arcs,sort_keys=True,allow_nan=False,indent=2)+'\n'
            if len(arc_data.encode('utf-8'))>4*1024*1024:
                raise RuntimeError('Derived arcs exceed frozen 4-MiB diagnostic cap')
            (a.out/'NONCNS_ARCS.json').write_text(arc_data,encoding='utf-8')
        status = 'COMPLETE'

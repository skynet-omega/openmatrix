...
def analizar(principal,afroot,out):
    ...
    try:
        ...
        for a in ARMS:
            ...
            exigir(t['sensors_used'].shape==(335,2),'Se requieren 335 entradas bilaterales por intervalo')
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for arm,c in zip(ARMS,coeff):
            ...
            trace=dict(ms=ms,ORN_ids=np.array([900,901]),ORN_q=orn,monitored_ids=np.array([523769,10360]),monitored_q=.2+.01*net,
                       qpos=qpos,neural_steering_applied=np.ones(336)*.1,sensors_used=orn[1:]-.1)

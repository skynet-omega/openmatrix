...
extern "C" __global__ void norm(const double*a,const double*b,int n,int normn,double atol,double rtol,double*err,int*flag,int*domain,const double*lower,const double*upper){__shared__ double z[256];int i=blockIdx.x*blockDim.x+threadIdx.x;double e=0;if(i<n){if(!isfinite(a[i])||!isfinite(b[i]))atomicOr(flag,1);if(b[i]<lower[i]||b[i]>upper[i])atomicOr(domain,1);if(i<normn)e=fabs(a[i]-b[i])/(3*(atol+rtol*fmax(fabs(a[i]),fabs(b[i]))));if(!isfinite(e)){atomicOr(flag,1);e=0;}}z[threadIdx.x]=e;__syncthreads();for(int k=128;k;k/=2){if(threadIdx.x<k)z[threadIdx.x]=fmax(z[threadIdx.x],z[threadIdx.x+k]);__syncthreads();}if(threadIdx.x==0)atomicMax((unsigned long long*)err,__double_as_longlong(z[0]));}
...
class NativeGraph:
 def __init__(self,initial,coefficient,*,rtol,atol,norm_size,project=None,freeze=None,native_library=None,state_bounds=(0.,1.)):
  ...
  # Per-coordinate model contract. The default preserves the legacy normalized
  # adapter; new models can declare negative or one-sided/unbounded variables.
  if not isinstance(state_bounds,(tuple,list)) or len(state_bounds)!=2:raise ValueError('Lower and upper state bounds required')
  self.lower_host=np.broadcast_to(np.asarray(state_bounds[0],dtype=float),(self.n,)).copy();self.upper_host=np.broadcast_to(np.asarray(state_bounds[1],dtype=float),(self.n,)).copy()
  if np.isnan(self.lower_host).any() or np.isnan(self.upper_host).any() or np.any(self.lower_host>self.upper_host):raise ValueError('Invalid model state bounds')
  ...
  with cp.cuda.using_allocator(self.pool.malloc),self.stream:
   ...
   if not bool(cp.isfinite(self.x).all()) or bool(cp.any((self.x<self.lower)|(self.x>self.upper))):raise ValueError('Initial state outside model bounds')
   mod=cp.RawModule(code=KERNEL);self.check=mod.get_function('check_values');self.norm=mod.get_function('norm');self.summary=mod.get_function('status');self.relax_kernel=mod.get_function('relax_fp64');self.grid=((self.n+255)//256,)
 ...
 def coeff(self,y,frac):
  ...
  return z,a,b
 def relax_update(self,z,a,b,scale):
  for value in (z,a,b):
 ...
 def midpoint(self,y,frac,start):
  z,a,b=self.coeff(y,start);middle=self.relax_update(z,a,b,-.5*frac)
  _,a,b=self.coeff(middle,start+.5*frac);out=self.relax_update(z,a,b,-frac)
 ...
 def advance(self,ns,next_ns,min_ns,max_ns,budget=30,boundaries=None):
  ...
  if code!=0:
   ...
   try:
    ...
    bad=np.flatnonzero(~np.isfinite(attempted)|(attempted<self.lower_host)|(attempted>self.upper_host))

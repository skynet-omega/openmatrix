"""Bounded four-arm current-pipeline flow experiment with optional read-only observer."""
...
from flow_observer import FlowObserver, install
...
def main() -> int:
    ...
    try:
        ...
        if a.observe == 'on':
            ...
            observer = FlowObserver(h,metadata,a.out/'flow')
            undo = install(session,observer)
        frozen = a.out/'executed_sources'
        frozen.mkdir()
        files = [HERE/'PLAN.json',HERE/'run_flow.py',HERE/'flow_observer.py',

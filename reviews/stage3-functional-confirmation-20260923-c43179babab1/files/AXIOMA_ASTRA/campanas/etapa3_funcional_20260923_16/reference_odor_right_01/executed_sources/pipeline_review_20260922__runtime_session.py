...
import sys,threading,inspect,hashlib
...
class RuntimeSession:
 def __init__(self,brain,profile):
  ...
  try:
   ...
   installed=b.advance.__func__
   if installed.__module__!=('device_cell' if profile=='causal_cuda' else 'native_cell'):raise RuntimeError('Wrong membrane implementation installed')
   if not self.adapter.event_boundaries or self.events.step.__self__ is not self.adapter:raise RuntimeError('Wrong CNS event executor installed')
   self.implementations={}
   for name,fn in (('membrane',installed),('CNS',self.events.step.__func__),('PN',brain._online_source.advance)):
    source=Path(inspect.getsourcefile(fn)).resolve()
    self.implementations[name]={'module':fn.__module__,'source':str(source),'sha256':hashlib.sha256(source.read_bytes()).hexdigest()}
  except BaseException:
   self.close();raise
 def report(self):
  return {'profile':self.profile,'installed_implementations':self.implementations,'event_boundaries':True,'geometry_compression':False,'coupling_ns':125000,'cell':None if self.cell is None else self.cell.report,'CNS':None if self.adapter is None else self.adapter.report,'PN':self.pn,'events':None if self.events is None else self.events.report,'partition':self.partition}
 def close(self):

...
def install(brain):
    ...
    def recorded_lif(v,r,c,q,vinf,rate,reset,threshold,caps,tau,dt,*args):
        ...
        clips,t,j=lif_record(v,r,c,q,vinf,rate,reset,threshold,caps,tau,dt,*args)
        rr,cc=np.where(np.isfinite(t));engine.active.add(t[rr,cc],engine.other[rr].astype(np.int64),j[rr,cc]);return clips
    def recorded_commit(b,v,g,ns):
        ...
        if len(rows):engine.active.add(np.full(len(rows),(b.elapsed_ns-engine.start_elapsed)*1e-9),engine.gamma[rows],delta[rows])

...
PLAN={'schema':'gaussian_intensity_pilot_v1','fields':['plus','minus'],
      'preparation_ms':40,'trial_ms':400,'source_offset_baselines':[.5,2.],
...
class FixedGaussian:
    ...
    def sample(self,data,source_mm,sigma_mm):
        ...
        need(0<=elapsed<=400*DT and elapsed%DT==0,'Reloj fuera del piloto')
...
class Intervals:
    ...
    def before(self,obj):
        need(not self.invalid and not self.armed and self.n<400,'Auditor inválido/reentrante')
    ...
    def export(self,path):
        save(path,{'field':self.field.metadata(),'prepared_pose_guard':self.prepared_pose_guard,'intervals':self.records,
                   'complete':self.n==400 and not self.invalid and not self.armed,

...
def source_check(campaign):
    ...
    need(PLAN['schema']=='stage4_mirrored_finite_source_prospective_v1' and
...
def decide(runs,invalid=False):
    ...
    def input_ok(m):
        return (m['late_signed_LR_plus']>threshold['late_300_400ms_signed_LR_plus_min_exclusive'] and
    ...
    def output_ok(m):
        return (m['command_L1_deg']>threshold['command_L1_pair_deg_min_exclusive'] and
                m['signed_command_plus_minus_deg']>threshold['signed_command_plus_minus_deg_min_exclusive'] and
                m['yaw_plus_minus_last_deg']>threshold['yaw_plus_minus_deg_min_exclusive'])
    if not input_ok(native):
        return {'rival':'C','classification':'DESCARTADO_EN_ESTE_CONTRATO','reason':'Opposite late lateral input not delivered','native_pair':native}
    if not output_ok(native):
        return {'rival':'B','classification':'DESCARTADO_EN_ESTE_CONTRATO','reason':'Consumed opposite lateral input without correctly signed material command and yaw in 400 ms','native_pair':native}
    ...
    okay=(input_ok(reference) and output_ok(reference) and

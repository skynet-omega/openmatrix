"""Paired one-second sensory replay; identity before lateral-contrast ablation."""
...
CAPTURE_EPOCHS = ('preparacion:1','preparacion:40','ensayo:1','ensayo:100',
                  'ensayo:400','ensayo:700','ensayo:900','ensayo:1000')

def source_files() -> tuple[Path,...]:
    return (HERE/'PLAN.json',HERE/'run_replay.py',HERE/'verify_pair.py',HERE/'replay_boundary.py',
            ROOT/'motor_nuevo/gaussian_prepared_semantics_20260923/compare_prepared.py',
            PRIOR/'CAMPOS.json',PRIOR/'reference/ORN_INDEX_MAP.json',
            PRIOR/'preflight_reference_guard.py',PRIOR/'REFERENCE_POSE_GUARD_01.json',
...
def main() -> int:
    ...
    p.add_argument('--field', choices=('minus',), default='minus')
    p.add_argument('--mode', choices=('identity','no_contrast'), required=True)
    p.add_argument('--engine', choices=('causal_cuda','reference_cuda'), default='causal_cuda')
    a = p.parse_args()
    contract=json.loads((HERE/'PLAN.json').read_text())
    if a.mode == 'no_contrast':
        from verify_pair import verify_run
        control=verify_run(HERE/'identity_01','identity')
        if control['identity_exact'] is not True:
            raise RuntimeError('Identity control not reproduced; intervention forbidden')
    prep = 40
    trial_ms = 1000
    max_wall = 4300
    ...
    try:
        ...
        atomic_json(a.out/'RUN_CONTRACT.json',{
            'schema':'stage4_sensory_contrast_pair_v1','mode':a.mode,'field':a.field,'engine':a.engine,
            ...
            'source_mode':'Replay of consumed donor tape; physical Gaussian recorded separately; no spatial feedback' ,
        ...
        gaussian_specs=json.loads((PRIOR/'CAMPOS.json').read_text())
        ...
        for phase, duration in [('preparacion',prep),('ensayo',trial_ms)]:
            if phase == 'ensayo':
                ...
                check=compare_preparation(a.out/'prepared_state',PRIOR/'native_minus_02/prepared_state')
                atomic_json(a.out/'PREPARATION_EQUALITY.json',check)
                if not check['scientific_state_exact']:
                    raise RuntimeError('Donor preparation differs before intervention')
                ...
                auditor=install_replay(obj,field,a.field,gaussian_specs[a.field],rows[-1],geometry_path=geom_path,
                                       mode=a.mode,donor=PRIOR/'native_minus_02/traces.npz',
                                       donor_sha=contract['donor_trace_sha256'])
                yaw0 = d.yaw_grados(obj.body.data.qpos)
            for k in range(duration):
                ...
                row['concentracion_fisica'] = (row['concentracion_campo'].copy() if phase=='preparacion'
                    else auditor.field.physical.sample(obj.body.data,None,None)['concentration'].copy())
                if phase == 'ensayo':
        ...
        if auditor is not None:
            try:auditor.export(a.out/'REPLAY_INTERVALS.json')
        ...
        atomic_json(a.out/'RESULT.json',{'status':status,'error':error,'cleanup_errors':cleanup,
            'field':a.field,'mode':a.mode,'engine':a.engine,'observer':'on',
            ...
            'reason':'Paired sensory replay identifies contrast contribution only; not closed-loop navigation or feedback.'})

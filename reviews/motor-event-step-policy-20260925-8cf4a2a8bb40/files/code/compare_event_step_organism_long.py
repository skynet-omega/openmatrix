...
def run(base, candidate, output, corrupt=False):
    plan_path = HERE/'EVENT_STEP_PLAN_43.json'
    ...
    for mode, folder in (('baseline',base),('candidate',candidate)):
        ...
        need(result['completed_trial_ms'] == 20 and result['completed_preparation_ms'] == 0,
             'Run duration')
    ...
    need(len(be['blocks']) == len(ce['blocks']) == 320, 'Block schedule changed')
    ...
    report['status'] = 'PASS_NECESSARY_TWENTY_MS_PREFLIGHT' if safety and speed else 'FAIL_PROSPECTIVE_GATE'
    report['scope'] = ('One sham20ms organism pair, same preparation and frozen equations, '

 ...
 try{
  ...
  while(used<end){
   ...
   if(e<=1.){
    ...
    // A mandatory event may truncate an otherwise accepted proposal. Carry its
    // pre-cut size forward only when the same small-error growth rule applies;
    // the next loop still clips at the next event and checks its own error.
    long proposal=std::min(maxstep,std::max(minstep,(long)std::floor(h*1e9*(e<.1?2:1))));
    if(ix<ne && stop<end && h==available && h<requested && e<.1)
     proposal=std::min(maxstep,std::max(proposal,*next));
    *next=proposal;

"""Bounded full-organism Gaussian intensity/history gate, not source navigation."""
...
def source_files() -> tuple[Path,...]:
    return (HERE/'PLAN.json',HERE/'run_gaussian.py',
...
def require_sources() -> dict:
    ...
    if (plan['fixed_inputs']['local_pose_guarded_helper_sha256']!=frozen[str((GAUSSIAN/'gaussiano_400.py').resolve())]
        ...
        raise RuntimeError('Stage4 plan, geometry, observer and source lock disagree')
...
def main() -> int:
    ...
    try:
        ...
        atomic_json(a.out/'RUN_CONTRACT.json',{
            'schema':'stage4_gaussian_gate_run_v1','field':a.field,'engine':a.engine,
            ...
            'biological_parameters_changed':False,'interface_intervention':'PN629 disabled before preparation; Gaussian source installed after preparation','stage3_admission':False,'stage4_admission':False,
        ...
        _,gaussian_specs=geometry(geom_path)
        ...
        for phase, duration in [('preparacion',prep),('ensayo',trial_ms)]:
            if phase == 'ensayo':
                ...
                initial_pair_abs=float(np.max(np.abs(first['plus']-first['minus'])))
                bound=json.loads((HERE/'PLAN.json').read_text())['preflight']['initial_plus_minus_concentration_max_abs']
                if not np.isfinite(initial_pair_abs) or initial_pair_abs>bound:
                    raise ValueError('Initial Gaussian pair differs beyond the frozen preparation bound')
                atomic_json(a.out/'GAUSSIAN_INITIAL_GUARD.json',{'plus':first['plus'].tolist(),
                            'minus':first['minus'].tolist(),'pair_max_abs':initial_pair_abs,'limit':bound})
        ...
        atomic_json(a.out/'RESULT.json',{'status':status,'error':error,'cleanup_errors':cleanup,
            ...
            'reason':'Gaussian input/history diagnostic only; not source navigation or biological equivalence.'})

...
def library():
    lib=ct.CDLL(str(H/'libopenmatrix_blocks.so'))
...
class Implicit:
    def __init__(self,spec,profile='precise',time0=0.,state=None,implementation='python'):
        started=time.perf_counter();require(profile in PROFILES,'profile')
        require(implementation=='python','block candidate keeps Python callbacks to isolate preconditioner');self.implementation=implementation
        ...
        self.blocks=Blocks(self.model,self.operator,self.stream)
        self.exception=None;self.calls=[0]*6;self.deadline=float('inf');self.unowned=[]
        with self.stream:
    ...
    def _call(self,kind,t,yp,vp,op,gamma):
        try:
            ...
            with self.stream:
                if kind==5:
                    self.operator.flag.fill(0);self.blocks.prepare(t,self.view(yp),gamma)
                    require((int(self.blocks.flag.get(stream=self.stream)[0])&1)==0 and int(self.operator.flag.get(stream=self.stream)[0])==0,'nonfinite block preparation')
                    return 0
                ...
                if kind in (0,1):
                    ...
                    self.blocks.solve(t,x,gamma,v,out)
                    require((int(self.blocks.flag.get(stream=self.stream)[0])&1)==0,'nonfinite block solve')
    ...
    def stats(self):
        ...
        return dict(zip(['steps','rhs_evaluations','nonlinear_iterations','linear_iterations','error_test_failures'],list(out)))|{'callbacks':self.calls.copy(),'implementation':'blocks','block_setups':self.blocks.setups,'block_factorizations':self.blocks.factorizations,'block_solves':self.blocks.applies}|(self.operator.stats() if self.implementation=='native' else {})
    def close(self):

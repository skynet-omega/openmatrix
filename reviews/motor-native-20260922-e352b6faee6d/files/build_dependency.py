...
def main():
    ...
    run(['/usr/bin/g++-12','-std=c++17','-O3','-shared','-fPIC','bridge.cpp','-I'+str(install/'include'),'-L'+str(install/'lib'),'-Wl,-rpath,$ORIGIN/vendor/install/lib','-lsundials_arkode','-lsundials_nveccuda','-lsundials_sunlinsolspgmr','-lsundials_core','-lcudart','-lcuda','-lnvrtc','-o','libopenmatrix_ark.so'],'build_bridge.log')

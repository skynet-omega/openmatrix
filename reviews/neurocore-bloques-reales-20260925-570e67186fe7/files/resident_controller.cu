...
__global__ void prepare(Control* c) {
  ...
  c->attempted_end_s = h == available ? stop : c->used_s + h;
  c->clock[0] = c->used_s;
  c->clock[1] = h;
  // ABI 2: the authorized endpoint is data, never reconstructed by a model.
  c->clock[2] = c->attempted_end_s;
}
...
extern "C" int resident_abi_version() { return 2; }

extern "C" void* resident_create_v2(void* captured_trial_graph, void* stream,

**Elegiría A: calificación funcional prospectiva de esta consulta, no rehabilitación del motor completo.** El límite de **0,002°** es defendible antes de observar las nuevas discrepancias; mide concordancia entre discretizaciones, **no error demostrado respecto de la solución exacta**. La campaña 15 conserva su carácter exploratorio y el FAIL microscópico no cambia.

Un matiz causal: calculando desde los recibos, el derecho mejora absolutamente **−0,05909044°**, pero el contraste derecho−sham cambia solo **−0,00717020°**: de −0,07538832° a −0,08255852°. La retirada puede rescatar orientación reduciendo un sesgo común; todavía no demuestra reparación de la selectividad PN. El 99,8 % corresponde al replay corporal, no a retirar el lector con cerebro, propiocepción y mundo acoplados.  

## A/B/C: decisión y límites

| Vía | Qué permitiría | Falsador |
|---|---|---|
| **A. Robustez funcional** | Comparar cuatro brazos native/reference durante 400 ms, preservando las KC y sus diferencias. | El efecto, signo, apoyo o trayectoria no sobrevive al presupuesto numérico. |
| **B. `reference_cuda`** | Adoptarlo si obtiene mejor evidencia de convergencia. | Reloj global y mayor coste no demuestran precisión; no elegirlo por dar mejor giro. |
| **C. Semántica de eventos** | Priorizarla si el muestreo impide estabilizar la consulta. | Localizar el defecto con inputs/pasos iguales; no sustituir estados por los de referencia. |

**A es legítima con un nombre y alcance explícitos:** orientación funcional de este modelo/protocolo/decodificador. Sería rescate indebido afirmar que pasó el antiguo gate, garantizar memoria futura o elegir después otra ventana/tolerancia. No exigiría identificar fisiológicamente cada parámetro del lector; sí declarar su escala **5°/s·tanh(250Δq)**. El antecedente exige dirección, controles, retirada de lectura, apoyo y continuación, no identidad microscópica universal. 

## Contrato que congelaría

En todos los instantes guardados a 1 ms, sin optimizar el alineamiento:

\[
E_\psi=\max_k|\Theta_k^N-\Theta_k^R|\le0,002^\circ,
\qquad
E_\omega=\sum_k|\omega_k^N-\omega_k^R|\Delta t\le0,002^\circ.
\]

El segundo impide ocultar comandos discrepantes por cancelación; **no es una cota del error corporal**. Se conserva la fórmula y el desfase de 1 ms. DN, flujo y eventos se informan separadamente, sin renombrar entradas abstractas como pA. No certifica errores entre muestras. 

Para la **nueva observación final a 400 ms** propongo tamaño material \(\delta=0,02^\circ\) y reserva \(\epsilon=0,002^\circ\): en ambos perfiles, \(\Theta_L>\epsilon\), \(\Theta_R<-\epsilon\) y, para \(c\in\{\text{sham, uniforme}\}\),

\[
\Theta_L-\Theta_c>\delta+2\epsilon,\qquad
\Theta_c-\Theta_R>\delta+2\epsilon.
\]

**0,024° es una propuesta nueva de margen, no el criterio histórico de mejora contra el padre.** El factor 2 reserva la discrepancia de dos brazos; sham se informa sin restarlo al controlador. Estas reservas no transforman la concordancia en una cota frente a la solución exacta.

## Confirmación mínima y finita

1. Cerrar izquierda/uniforme pendientes sin alterar campaña 15. Publicar el contrato antes de nuevas referencias. Ejecutar **derecha reference primero**; detener si falla. Si pasa, sham, izquierda y uniforme.
2. Retirar solo la lectura angular: imponer \(\omega=0\) después del lector, manteniendo avance, cerebro, propiocepción y exposición espacial. Dos brazos L/R desde el mismo preparado, 400 ms. Predicción prefijada: \(|(\Theta_L-\Theta_R)/2|\le0,004^\circ\), frente al contraste intacto. El replay corporal no sustituye este control.
3. Una continuación del brazo derecho desde su snapshot de 100 ms hasta 400 ms, contra la ejecución ininterrumpida del mismo perfil: restauración íntegra de estado/operador/eventos pendientes y robustez observable.

Máximo **siete ejecuciones**, 2.050 s cada una, en otro contrato: cuatro referencias, dos retiradas y una continuación. Parar al primer fallo; no ensayar mallas hasta conseguir PASS. Si todo se cumple, permitiría declarar **etapa 3 funcional acotada**; no motor general validado ni navegación hacia una fuente.

## Código: `contrato_funcional.py`

Lee pares NPZ/JSON, sin simular. **No adjudica retirada ni continuación.** Comprueba apoyo elemental: cuerpo no invertido, contacto y fuerza normal positiva por muestra; no estabilidad entre muestras.
...
```bash
python contrato_funcional.py --plan pares.json --out revision_funcional_01
```

`pares.json` empieza con derecha; añadir los otros pares sin cambiar criterios:

```json
{"schema":"orientation_functional_v1","registered_utc":"REGISTRO_UTC_REAL_ANTES_DE_REFERENCIAS","pairs":{"odor_right":{"causal":"/ruta/native/full_odor_right_01","reference":"/ruta/reference/full_odor_right_01"}}}
```

Publicar fecha y hash reales antes de ejecutar; el programa no autentica por sí solo la precedencia temporal. Salida 0 significa concordancia de los pares disponibles, **no etapa 3**. Ausencia de `EVENT_AUDIT` se informa, nunca se sustituye por igualdad.

**Ejecutado:** ocho controles CPU de software, no neuronales, en 2,02 s. Incluyen rechazo de un transitorio de 0,0021° oculto en el endpoint, comandos compensados pero distintos, lag incorrecto y cuatro giros del mismo signo.

**Leídos completos:** README, ARCHIVE, REVIEW_PLAN, PLAN, PARTIAL_REVIEW, ORIENTATION_SCOPE_ANTECEDENT, `analyze_long.py`, `run_set.py`, RIGHT_DECISION, RUN_CONTRACT/INTERVENTION del sham y `body_factorial_01/RESULT.json`. El ZIP falló por DNS: **no ejecuté sus NPZ, CUDA ni el organismo**. Los cálculos iniciales usan valores textuales de los recibos.

:chatgpt-content-reference{index="4"}[Fuentes, pruebas y resultados](sandbox:/mnt/data/AXIOMA_CONTRATO_FUNCIONAL_8B2D.zip).

...
class Waveform:
    ...
    def at(self,time,xp=np):
        ...
        if xp is not np and any(np.isfinite(p) and te<=time for p,te in zip(self.posts,self.times)):
        ...
        if xp is np and any(np.isfinite(p) and te<=time for p,te in zip(self.posts,self.times)):
            affected={int(self.rows[i]) for i,post in enumerate(self.posts) if np.isfinite(post) and self.times[i]<=time}
            for row in affected:

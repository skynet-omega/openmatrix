...
try:
 ...
 receipt.update(state='FAILED_OR_UNCERTAIN',error_type=type(exc).__name__,http_status=getattr(exc,'code',None))
 if hasattr(exc,'read'):
  body=exc.read(10000).decode('utf-8',errors='replace').replace(key,'[redacted]')
  if 'apikey_' not in body and 'Bearer ' not in body:(D/'error_body.txt').write_text(body)

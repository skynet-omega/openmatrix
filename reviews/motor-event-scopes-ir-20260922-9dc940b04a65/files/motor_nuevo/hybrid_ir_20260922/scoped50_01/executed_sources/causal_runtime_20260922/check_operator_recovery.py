...
sys.path.insert(0,str(HERE.parent/'native_hybrid_20260922'))
...
def install(h):
 ...
 _,u=pn_execution.install(h);undo.append(u);_,u=native_cell.install(h,events,compressed=False);undo.append(u)
...
for mode in ('continuous','cold','fault'):
 fault=mode=='fault'
 obj,*_=load(out/mode);h=obj.core.hybrid;events,adapter,undo=install(h)
 try:
  before={};old_advance=h.advance;body_before=obj.body.data.qpos.copy()
  operator=OperatorState(h,LEGACY_BINDINGS)
  if mode=='cold':
   frames=ParentSnapshotFrames(h);parent,published=frames.capture(h);source=copy.deepcopy(h._online_source.state_dict())
   h._restore_joint(parent,source,published);frames.release(parent)
   result['cold_lost_operator_fields']=operator.differences(h)
   for fn in reversed(undo):fn()
   operator.restore(h);events,adapter,undo=install(h);old_advance=h.advance
   result['cold_operator_restored_exact']=not operator.differences(h)
  ...
  if fault:
   ...
   result['fault_lost_operator_fields']=operator.differences(h)
   ...
   for fn in reversed(undo):fn()
   operator.restore(h);events,adapter,undo=install(h)
   result['fault_operator_restored_exact']=not operator.differences(h)
  if mode=='continuous':
   obj.step();expected=before['after']
  else:
   if mode=='cold':
    obj.step();before['retry']=before['after']
   else:
    try:obj.step()
   ...
   actual=before['after'] if mode=='cold' else h.state_dict()
   changes=same(expected,actual);result[mode+'_neural_retry_changed_paths']=changes
   from session_io import write_state
   write_state(out/'control_neural',expected);write_state(out/(mode+'_neural'),actual)
   aa,bb=dict(flatten(expected)),dict(flatten(actual));differences=[];material=[]
   for path in changes:
   ...
   result[mode+'_continuation_differences']=differences
   ...
   if fault and not np.array_equal(obj.body.data.qpos,body_before):raise RuntimeError('Body advanced despite neural failure')
   if fault:result['body_unchanged_after_failed_step']=True
 finally:
  for fn in reversed(undo):fn()
  obj.close()
 del obj,h,events,adapter,undo;gc.collect()
result.update(status='PASS',wall_s=time.perf_counter()-t,scope='Effective operator retained; continuous/cold/fault neural continuation tested. First CNS block failure after accepted GPU epoch: complete neural physical state restored; explicit runtime reconstruction and neural 1ms retry tested against predeclared execution bound; exact differences retained. Failed coupled session refuses reuse; body remains unadvanced. Automatic whole-organism resume is not implemented.')

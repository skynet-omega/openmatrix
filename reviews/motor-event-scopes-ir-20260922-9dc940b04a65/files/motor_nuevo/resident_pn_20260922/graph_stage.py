...
from resident_cyclic import CyclicTree
scatter=module.get_function('scatter_current');syn_current=module.get_function('synapse_current')
reduce_values=module.get_function('reduce_values');reduce_finish=module.get_function('reduce_finish');controls=module.get_function('controls')
...
class StageGraph:
 def __init__(self,p,syn,shift,q):
  ...
  self.tree=CyclicTree(p.backend.plan,shift);self.A=p.backend.csr((p.backend.parent.G+shift*p.backend.parent.M).tocsr());self.keep=[];self.reducers=[];self.control=cp.empty(11)
  def arrays():
  ...
  stream.synchronize()
 def reduce(self,x,mode):
  count=min(128,(len(x)+255)//256);i=self.reduce_count;self.reduce_count+=1
  if i==len(self.reducers):self.reducers.append((cp.empty(count),cp.empty(1)))
  partial,out=self.reducers[i]
  if len(partial)!=count:raise ValueError('Reduction shape changed')
  reduce_values((count,),(256,),(np.int32(len(x)),x,np.int32(mode),partial));reduce_finish((1,),(256,),(np.int32(count),partial,np.int32(mode),out));return out
 ...
 def launch(self):
  ...
  for iteration in range(2):
   ...
   cp.multiply(self.p.backend.C,self.shift,out=self.signed_work);cp.add(self.signed_work,self.diagonal,out=self.signed_work);signed.append(self.reduce(self.signed_work,1))
   cp.negative(z['r'],out=self.tree.rhs);cp.copyto(self.tree.diagonal,self.diagonal);self.tree.enqueue();cp.bitwise_or(self.flag,self.tree.flag,out=self.flag)
   csr_kernel(((self.n+127)//128,),(128,),(np.int32(self.n),*self.A,self.tree.x,np.int32(1000000000),self.linear_product));cp.multiply(self.diagonal,self.tree.x,out=self.linear_diag);cp.add(self.linear_product,self.linear_diag,out=self.linear_residual);cp.add(self.linear_residual,z['r'],out=self.linear_residual);linears.append(self.reduce(self.linear_residual,0))
   cp.add(self.v,self.tree.x,out=self.v);norms.append(self.evaluate(self.v,self.evals[iteration+2]))
  ge=self.reduce(self.evals[3]['ge'],2);ce=self.reduce(self.evals[3]['ce'],2);controls((1,),(1,),(refnorm,*norms,ge,ce,*linears,*signed,self.flag,self.control))

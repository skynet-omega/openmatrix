...
def main():
    ...
    for engine in ('native',):
        off,on=read(engine+'_capture_off_01'),read(engine+'_capture_on_02')
        a,za=off;b,zb=on
        results={}
        for field in FIELDS:
            differences=[]
        ...
        same_files={name:sha(HERE/(engine+'_capture_off_01')/'final_state'/name)==sha(HERE/(engine+'_capture_on_02')/'final_state'/name)
    ...
    result={'schema':'kc_post_commit_capture_two_ms_neutrality_v1','engines':verdicts,
            ...
            'scope':'Full final scientific state tree and three auxiliary checkpoint files for causal engine only; excludes wall time/diagnostic buffers. Reference on/off was not budgeted after a failed capture run.'}

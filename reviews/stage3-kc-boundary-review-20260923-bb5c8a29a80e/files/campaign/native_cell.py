...
class Cell:
 def __init__(self,b,events,compressed=False):
  ...
  with cp.cuda.using_allocator(self.pool.malloc),self.stream:
   ...
   self.step_kernel=build_step(groups,certificate);mod=cp.RawModule(code=(HERE/'physical_events.cu').read_text(),options=('--fmad=false',));self.somatic=mod.get_function('commit_event');self.axonal=mod.get_function('commit_axon')

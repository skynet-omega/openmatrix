...
ROOT=HERE.parents[1]
PLAN=HERE/'CELL_KERNEL_PLAN_47.json'
...
def run(out):
    ...
    need(p['schema']=='cell_kernel_timing_plan_v2' and not out.exists(), 'Plan/output')
    ...
    context={'timings_ms':[],'host_advance_ms':[],'owner_delta_ms':[],
             'wrapped':0,'calls':0}
    def installed(session,*args,**kwargs):
        ...
        def timed_advance(self,*a,**kw):
            ...
            context['host_advance_ms'].append((time.perf_counter()-host_before)*1000)
            context['owner_delta_ms'].append((self.report['native_wall_s']-owner_before)*1000)
    ...
    report={'schema':'cell_kernel_timing_result_v2','plan_sha256':sha(PLAN),
            ...
            'per_call_host_advance_ms':context['host_advance_ms'],
            'per_call_owner_delta_ms':context['owner_delta_ms'],
            'sum_kernel_s':sum(context['timings_ms'])/1000,
            'sum_host_advance_s':sum(context['host_advance_ms'])/1000,
            'sum_owner_delta_s':sum(context['owner_delta_ms'])/1000,

...
def verify(root=HERE):
    root = Path(root)
    p = root/'EVENT_STEP_PAIR_LONG_RESULT.json'
    pair = json.loads(p.read_text())
    names = ('event_step_baseline_long_01', 'event_step_candidate_long_01')
    audit = [json.loads((root/n/'EVENT_AUDIT.json').read_text()) for n in names]
    ...
    for key, receipt_key in (('max_time_s', 'event_time_max_abs_s'),
        ...
        need(math.isclose(found[key], pair[receipt_key], rel_tol=1e-10, abs_tol=1e-15),
             'reported ' + key)
    capsule = np.load(root/'EVENT_STEP_LONG_CAPSULE.npz')
    ...
    return {'schema': 'event_step_strict_audit_v1', 'pair_sha256': sha(p),
            'audit_sha256': {n: sha(root/n/'EVENT_AUDIT.json') for n in names},
...
if __name__ == '__main__':
    ...
    arg.add_argument('--root', type=Path, default=HERE)

...
def main():
    ...
    p.add_argument('--ms',type=int,choices=(20,100,1000,5000),required=True)
    ...
    result={'mode':a.mode,'requested_ms':a.ms,'completed_ms':0,'field':a.field,
            'status':'STARTED','parameters_changed':False,'whole_neural_rewrite':False,
            'scope':'Real coupled organism; candidate replaces CNS executor only. PN/spatial solvers conserved.'}
    ...
    try:
        ...
        for k in range(a.ms):
            ...
            if time.perf_counter()-start>a.wall_limit-25:raise TimeoutError('saving prefix before wall limit')
        ...
        write_state(final/'session',obj.core.state_dict())
        ...
        result['sources']={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in
             [HERE/'run_real.py',HERE/'real_model.py',HERE/'graph_runtime.py',HERE/'resident_controller.cu',HERE/'libresident_controller.so',HERE/'PLAN.md']}

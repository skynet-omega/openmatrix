"""Lossless XOR-coded real release vectors plus two full same-time pairs."""
import argparse
import hashlib
import json
import resource
...
def run(plan_path, out):
    ...
    release_sha = hashlib.sha256(release.tobytes()).hexdigest()
    bits = release.view(np.uint64)
    first = bits[0].copy()
    ...
    path = out / "release_capsule_xor.npz"
    np.savez_compressed(path, release_first_bits=first, release_adjacent_xor=xor,
                        outdegree=outdegree, query_s=times, sample_query_ids=selected,
    ...
    manifest = {"schema": "release_reuse_xor_capsule_v1", "status": "COMPLETE",
                "capture_sha256": plan["capture_sha256"],
                "source_index_array_sha256": source_index_sha,
                "release_uncompressed_sha256": release_sha,

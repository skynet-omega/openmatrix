...
def main():
 ...
 for d in (a,b):
  if json.loads((d/'RESULT.json').read_text())['status']!='COMPLETE':raise ValueError('Incomplete input')
 snapshots={str(i):brain_comparison(a/f'brain_{i:02d}ms',b/f'brain_{i:02d}ms') for i in (1,5,20)}
 endpoint=qualify(a,b);perf={}
 for d in (a,b):
 ...
 result={'brain_snapshots':snapshots,'twenty_ms':endpoint,'performance':perf,'fine_one_ms_repeat':brain_comparison(T/'fine1562_01/brain_final',a/'brain_01ms'),'stage3_admission':False,'scope':'Bounded sham20ms numerical comparison. Fine1562 is a declared refinement, not a converged ground truth. No stage3 behavioral experiment.'}
 (T/'CANDIDATES.json').write_text(json.dumps(result,indent=2,allow_nan=False)+'\n')
 print(json.dumps({'snapshots':{k:{f:v[f] for f in ('normalized_state_max_abs','gate_max_abs','voltage_max_abs_mV','changed_discrete_fields')} for k,v in snapshots.items()},'historical_endpoint':endpoint['historical_partial_screen']['screen_pass'],'uncovered_fields':len(endpoint['uncovered_changed_numeric_fields']),'performance':perf},indent=2))

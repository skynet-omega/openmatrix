...
def traced(self,b,ns,drive,light):
 ...
 inputs={'times_s':np.asarray(waveform.times,dtype=float).tolist(),'source_waveform_rows':np.asarray(waveform.rows,dtype=int).tolist(),'jumps':np.asarray(waveform.jumps,dtype=float).tolist()}
...
try:code=run_pipeline.main(['--out',str(H/(sys.argv[1] if len(sys.argv)>1 else 'trace_smoke_01')),'--odor','sham','--engine','causal_cuda','--ms','1','--smoke-test'])
finally:

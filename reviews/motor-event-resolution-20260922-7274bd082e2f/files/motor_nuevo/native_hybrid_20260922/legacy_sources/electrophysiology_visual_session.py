"""Whole-organism physical edge assay with an optional local electrical hypothesis."""
...
SCHEMA = "matrix_electrophysiology_visual_session_v1"
...
SOURCES = tuple(CARDINAL_SOURCES) + ("electrophysiology_edge_world.py", "measured_t4_visual_brain.py", "gpu_measured_t4_visual_brain.py", "electrophysiology_visual_session.py")


class ElectrophysiologyVisualSession(CardinalVisualSession):
    @classmethod
    def from_cardinal(cls, path, condition="on_positive", calibration=None, tight=False):
        parent = CardinalVisualSession.load(path)
        obj = cls()
        obj.__dict__.update(parent.__dict__)
        try:
            rotation, centers = obj.eyes.pose()
            center = np.mean([centers[k] for k in sorted(centers)], axis=0)
            obj.light_world = ElectrophysiologyEdgeWorld(obj.time_ns, center, rotation, condition)
            if calibration is not None:
                ...
                kind = GpuMeasuredT4VisualBrain if obj.config["numerical_backend"] == "cuda_fp64" else MeasuredT4VisualBrain
                obj.hybrid = kind.adopt(obj.hybrid, types, calibration)
            ...
            obj.config.update(candidate="ELECTROPHYSIOLOGY_EDGE_ASSAY_v1", measured_t4_family=calibration is not None, visual_challenge=condition,
                tight_neural_numerics=bool(tight), biological_validation=False)
            obj.config["electrical_backend_identity"] = (
                obj.hybrid.backend_identity() if isinstance(obj.hybrid, GpuMeasuredT4VisualBrain)
                else {"backend": "cpu_fp64", "local_reversal": calibration is not None})
            obj.intervention = dict(previous_intervention=copy.deepcopy(parent.intervention),
                ...
                time_ns=obj.time_ns, operation="Physical ON/OFF screen; optional Mi9-to-T4 receptor reversal hypothesis from independent current-voltage data",
                ...
                fitting="Only declared T4 receptor reversal when calibration supplied; no fitting to visual response or body movement",
                calibration=copy.deepcopy(calibration),
                experiment_limits="Normalized monochrome light, freely moving male model, head-centered screen; source physiology immobilized females with a frontal-left screen and measured spectrum/luminance. Eight-second schedule is a geometric reconstruction, not verified raw acquisition timing.")
    ...
    def load(cls, path):
        ...
        brain_types = {kind.SCHEMA: kind for kind in (ReceptorVisualBrain, GpuReceptorVisualBrain, MeasuredT4VisualBrain, GpuMeasuredT4VisualBrain)}
        obj.hybrid = brain_types[state["hybrid"]["schema"]].from_state(obj.brain, state["hybrid"])
        if isinstance(obj.hybrid, GpuMeasuredT4VisualBrain) and obj.config["electrical_backend_identity"] != obj.hybrid.backend_identity():
        ...
        try:
            ...
            world_type=ElectrophysiologyEdgeWorld

...
from parallel_tree import ParallelTree
import cupy as cp
...
shift=solves[0][1]['shift'];old=p.backend._graph_plan;start=time.perf_counter();gpu=ParallelTree(old,shift);setup=time.perf_counter()-start
...
result['device_pool_bytes']=cp.get_default_memory_pool().total_bytes();(H/'GPU_SCREEN.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2))

...
source=textwrap.dedent(inspect.getsource(EliminationPlan.__init__)).replace('def __init__','def symbolic').replace('len(self.core)>256','len(self.core)>178838')
...
class Condensation:
 def __init__(self,backend,protected,shift):
  ...
  if not np.isfinite(shift) or shift<=0:raise ValueError('Positive finite shift required')
  self.backend=backend;self.shift=float(shift)
 ...
 def reduce(self,rhs):
  self.backend.assert_model()
  if rhs.shape!=self.backend.C.shape or not np.isfinite(rhs).all():raise ValueError('Finite full RHS required')
  return reduce_rhs(self.plan.steps,self.plan.pairs,self.d,self.values,rhs)
 ...
 def solve(self,rhs,diag):
  if diag.shape!=self.backend.C.shape or not np.isfinite(diag).all() or np.any(diag[self.plan.steps[:,0]]!=0):raise ValueError('Diagonal correction outside protected core')

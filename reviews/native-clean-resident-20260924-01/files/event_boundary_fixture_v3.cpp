...
class Publisher final : public Owner {
 public:
  Publisher(double at_end_first, double at_start_second, double simultaneous_second)
      : at_end_first_(at_end_first), at_start_second_(at_start_second),
        simultaneous_second_(simultaneous_second) {}
  ...
  void advance(Context& context) override {
    ...
    if (calls_ == 1 && simultaneous_second_ != 0.0) {
      context.publish(context.now_ns, "pulse", simultaneous_second_);
    }
    ++calls_;
  }
...
Fixture make_fixture(double at_end_first, double at_start_second,
                     double simultaneous_second = 0.0) {
...
int main() {
  try {
    ...
    auto simultaneous = make_fixture(0.0, 1.0, 2.0);
    simultaneous.session->tick();
    simultaneous.session->tick();
    require(simultaneous.receiver->total() == 3.0, "simultaneous event sum incorrect");
    require(simultaneous.session->context().events().size() == 2,
            "simultaneous events collapsed or duplicated");
    auto interior = make_fixture(0.0, 0.0);
    interior.session->tick();
    require(interior.receiver->total() == 0.0, "empty tick changed receiver");
    std::cout << "PASS boundary_end=1 boundary_start=1 retry=1 empty=1 simultaneous=1\n";
    return 0;

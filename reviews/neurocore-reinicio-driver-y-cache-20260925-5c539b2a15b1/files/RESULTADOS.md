...
desconocido) fueron rechazadas antes de ejecutar el cargador. El verificador
`check_checkpoint_guards.py` produjo el mismo SHA256 normal y con `-O`
(`CHECKPOINT_GUARDS.json`). Por ello, **el defecto
...
checkpoint. La instalación no explica el fallo por una mutación visible de esos
estados; las cachés privadas nuevas siguen siendo distintas por construcción.

...
def hardware_state():
    ...
    for path in Path('/proc').glob('[0-9]*/cmdline'):
        try:
            args=path.read_bytes().decode(errors='replace').split('\x00')
    ...
    try:
        ...
        observed=gpu.stdout.strip() if gpu.returncode==0 else 'UNAVAILABLE'
    except (OSError,subprocess.TimeoutExpired):observed='UNAVAILABLE'
    return {'python_processes':processes,'gpu_compute_apps':observed}


def main():
    p=argparse.ArgumentParser()
    p.add_argument('--mode',choices=('fp32','persistent_fp32'),required=True)
    p.add_argument('--audit-weights',action='store_true')
    ...
    p.add_argument('--precision-factor',type=float,choices=(1.,.1),default=1.)
    p.add_argument('--compact-state',action='store_true')
    a=p.parse_args();a.out.mkdir(parents=True,exist_ok=False)
    if a.audit_weights and (a.ms!=1 or a.mode!='persistent_fp32'):
        raise ValueError('Weight audit is a 1-ms preflight, never a timed benchmark')
    ...
    result={'mode':a.mode,'requested_ms':a.ms,'completed_ms':0,'field':a.field,
            ...
            'scope':'Real coupled organism; C++/CUDA resident CNS executor with FP32 or FP64 dominant CSR. PN/spatial solvers conserved.',
            'cns_csr_precision':a.mode,'cns_integrator_precision':'fp64',
            'model_specific_precision':'fp64'}
    result['sources']={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in
         [HERE/'run_real.py',HERE/'real_model.py',HERE/'fp32_operator.py',HERE/'fast_fp32_coefficient.cu',
          HERE/'persistent_weights.py',HERE/'persistent_fp32.cu',HERE/'model_weight_writers.py',
          HERE/'graph_runtime.py',HERE/'resident_controller.cu',HERE/'libresident_controller.so',HERE/'PLAN.md']}
    records=[];arrays={};sampled=[]
    result['hardware_samples']=[{'step':0,**hardware_state()}]
    ...
    try:
        ...
        undo=install(a.mode,audit_weights=a.audit_weights)
        ...
        for k in range(a.ms):
            ...
            if (k+1)%25==0:result['hardware_samples'].append({'step':k+1,**hardware_state()})
        ...
        if operator is not None and operator.mirror is not None:
            result['weight_mirror']=operator.mirror.report()
            result['weight_mirror']['writer_groups']=operator.writer_groups
            if result['weight_mirror']['audit_mismatches']:
                raise RuntimeError('Undeclared or stale FP32 weights in the real operator')
        ...
        if session is not None:
            ...
            if session.adapter.core:
                core=session.adapter.core
                result['cns_resident_s']=core.resident_wall_s
                result['rhs_evaluations']=4*(core.steps+core.rejected)

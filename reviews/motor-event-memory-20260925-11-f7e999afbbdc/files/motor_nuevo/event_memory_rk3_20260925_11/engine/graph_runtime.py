...
KERNEL = r'''
extern "C" __global__ void left_clock(const double*c,double*out) {
 if(threadIdx.x==0 && blockIdx.x==0) {out[0]=nextafter(c[0]+c[1],c[0]);out[1]=0.;}
}
...
class GraphRK23:
    def __init__(self, initial, rhs, *, rtol, atol, norm_size=None,
        ...
        dll.resident_create.argtypes=[ptr]*6+[lng,lng];dll.resident_create.restype=ptr
        ...
        with cp.cuda.using_allocator(self.pool.malloc),self.stream:
            ...
            self.clock=cp.asarray([0.,1e-6]);self.left=cp.zeros(2)
            ...
            self.left_kernel=self.module.get_function('left_clock')
            ...
            try:
                self.handle=dll.resident_create(raw,self.stream.ptr,self.clock.data.ptr,
    ...
    def trial(self):
        ...
        self.fine=self.projected(high,self.clock,1.)
        ...
        self.left_kernel((1,),(1,),(self.clock,self.left))

"""Bounded real-organism comparison of resident RK3 event proposal policies."""
import os
...
def main():
    ...
    p.add_argument('--engine',choices=('control','event_memory'),required=True)
    p.add_argument('--ms',type=int,choices=(100,1000),default=100)
    ...
    ENGINE=PARENT if a.engine=='control' else HERE/'engine'
    source_lock=json.loads((HERE/'SOURCES.json').read_text())
    need(all(sha(k)==v for k,v in source_lock.items()),'Frozen trial sources changed')
    ...
    result={'status':'STARTED','engine':a.engine,'requested_ms':a.ms,'completed_ms':0,
            ...
            'scope':'Whole organism from campaign40 preparation; same Gaussian and motor filter',
            'source_checkpoint':str(PREFIX/'prepared_state'),
            'source_trace_sha256':sha(PREFIX/'traces.npz'),
            'runner_sha256':sha(__file__),'restore_sha256':sha(HERE/'restore_prepared.py'),'plan_sha256':sha(HERE/'PLAN.json'),'sources_sha256':sha(HERE/'SOURCES.json')}
    ...
    try:
        ...
        if a.engine in ('control','event_memory'):
        ...
        for k in range(1,a.ms+1):
            ...
            row.update(neural_command_raw_rad_s=motor.raw,motor_filter_applied_rad_s=motor.applied,
                       motor_filter_state_rad_s=motor.filtered,wind_torque_native=0.)
            ...
            need(motor.wind_substeps==0,'Wind must not occur before end of first second')
            rows.append(row)
            if k in (1,100,1000):states.append(collect(obj,cp))
            if k in (100,1000):

"""Campaign40 prepared-state restoration, derived from its proven continuation loader.
...
def restore(obj,folder):
    ...
    try:
        ...
        # This harness restores the recorded preparation BEFORE the Gaussian ON.
        # Keep the historical world, body, operator, and all neural state above.
        from static_field import StaticLateralField
        required={'arm','installed_ns','center_mm','odor_axis',
                  'first_ON_after_install_ms','live_geometry',
                  'stationary_world_field','calibrated_optogenetic_drive'}
        require(set(boundary)==required and boundary['arm']=='sham',
                'Expected campaign40 prepared sham boundary')
        ...
        field.center=np.asarray(boundary['center_mm'],dtype=float).copy()
        field.axis=np.asarray(boundary['odor_axis'],dtype=float).copy()
        field.onset_ms=float(boundary['first_ON_after_install_ms'])
        require(core.time_ns==field.origin_ns+40_000_000 and
                core.world.next_event==len(core.world.events),
                'Wrong preparation clock or pending historical event')
        require(field.metadata()==boundary,'Prepared boundary metadata changed')

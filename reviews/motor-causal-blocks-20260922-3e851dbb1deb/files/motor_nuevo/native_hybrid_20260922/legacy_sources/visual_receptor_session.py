"""Whole-CNS receptor intervention; archived visual/transmission families stay immutable."""
...
SCHEMA = "matrix_visual_receptor_session_v1"
...
SOURCES = tuple(TRANSMISSION_SOURCES) + ("receptor_visual_brain.py", "gpu_receptor_visual_brain.py", "visual_receptor_session.py")


class VisualReceptorSession(VisualTransmissionSession):
    @classmethod
    def from_precision(cls, path, condition="static", kinetic=True, tight=False):
        if not kinetic:
            raise ValueError("Receptor experiment requires the matched finite-transmission parent.")
        obj = super().from_precision(path, condition=condition, kinetic=True, tight=tight)
        try:
            table = pd.read_parquet(ROOT/"data/male_v10/nodes.parquet").set_index("bodyId")
            node_types = table.loc[obj.brain.node_ids, "type"].fillna("").to_numpy(str)
            before = obj.hybrid.state.copy()
            backend = GpuReceptorVisualBrain if obj.config["numerical_backend"] == "cuda_fp64" else ReceptorVisualBrain
            obj.hybrid = backend.adopt(obj.hybrid, node_types)
            obj.hybrid.sync_plastic_weights(obj.plasticity)
            np.testing.assert_array_equal(before, obj.hybrid.state)
            obj.config.update(candidate="VISUAL_RECEPTOR_R8_MI1_v1", receptor_condition="R8p_R8y_to_Mi1_excitatory",
                receptor_validation=False)
            obj.intervention = dict(previous_intervention=copy.deepcopy(obj.intervention),
                ...
                time_ns=obj.time_ns, operation="Postsynaptic receptor hypothesis on canonical R8p/R8y to Mi1 pairs",
                all_neural_and_body_state_preserved_exactly=True,
                anatomical_weights_and_topology_preserved=True,
                effective_sign_changed=True,
                evidence_scope="R8 cotransmission and Mi1 receptor transcript evidence; this pair's electrical sign is a model hypothesis",
                fitting="No response amplitude, gain, kinetics, optical or body parameter fitted",
                excluded="R8d, R8_unclear and R7 retained without intervention")

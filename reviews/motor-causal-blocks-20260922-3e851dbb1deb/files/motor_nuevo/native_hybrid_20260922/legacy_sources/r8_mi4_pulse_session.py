"""Whole-organism retinal pulse with an explicit R8-to-Mi4 receptor hypothesis."""
...
SCHEMA = "matrix_r8_mi4_pulse_session_v1"
...
SOURCES = tuple(ELECTRICAL_SOURCES) + ("retinal_pulse_world.py", "r8_mi4_visual_brain.py", "gpu_r8_mi4_visual_brain.py", "r8_mi4_pulse_session.py")


class R8Mi4PulseSession(ElectrophysiologyVisualSession):
    @classmethod
    def from_electrophysiology(cls, path, condition="dark_bright_dark", enabled=False, tight=False):
        parent = ElectrophysiologyVisualSession.load(path)
        obj = cls()
        obj.__dict__.update(parent.__dict__)
        try:
            rotation, centers = obj.eyes.pose()
            center = np.mean([centers[k] for k in sorted(centers)], axis=0)
            obj.light_world = RetinalPulseWorld(obj.time_ns, center, rotation, condition)
            types = pd.read_parquet(ROOT/"data/male_v10/nodes.parquet").set_index("bodyId").loc[obj.brain.node_ids, "type"].fillna("").to_numpy(str)
            kind = GpuR8Mi4VisualBrain if obj.config["numerical_backend"] == "cuda_fp64" else R8Mi4VisualBrain
            obj.hybrid = kind.adopt(obj.hybrid, types, enabled=enabled)
            ...
            obj.config.update(candidate="R8_MI4_PULSE_ASSAY_v1", r8_mi4_enabled=bool(enabled), visual_challenge=condition,
                tight_neural_numerics=bool(tight), biological_validation=False)
            obj.config["electrical_backend_identity"] = (
                obj.hybrid.backend_identity() if isinstance(obj.hybrid, GpuR8Mi4VisualBrain)
                else {"backend": "cpu_fp64", "r8_mi4_enabled": bool(enabled)})
            obj.intervention = dict(previous_intervention=copy.deepcopy(parent.intervention),
                ...
                time_ns=obj.time_ns, operation="Physical dark-bright-dark pulse and optional R8p/y-to-Mi4 postsynaptic receptor interpretation",
                ...
                fitting="No magnitude fitted. Existing R8p/y-to-Mi4 pairs use abs(stored W) if enabled, otherwise retain prior sign. T4 reversal and all earlier receptor hypotheses preserved",
                r8_mi4_enabled=bool(enabled),
                experiment_limits="Own three-second pulse assay, not replication of source physiology: normalized monochrome light, freely moving male model, fixed head-centered screen. RNA nondetection is not proof of protein absence; effective sign is a candidate, not measured current.")
    ...
    def load(cls, path):
        ...
        brain_types = {kind.SCHEMA: kind for kind in (R8Mi4VisualBrain, GpuR8Mi4VisualBrain)}
        obj.hybrid = brain_types[state["hybrid"]["schema"]].from_state(obj.brain, state["hybrid"])
        if isinstance(obj.hybrid, GpuR8Mi4VisualBrain) and obj.config["electrical_backend_identity"] != obj.hybrid.backend_identity():

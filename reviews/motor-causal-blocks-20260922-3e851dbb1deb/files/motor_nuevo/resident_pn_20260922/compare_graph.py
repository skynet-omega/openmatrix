...
out=H/sys.argv[1];out.mkdir(exist_ok=False);parent=PortablePN(P/'capture_01');candidate=ResidentPN(P/'capture_01')
from graph_step import advance_graph_resident
candidate.advance=lambda dt,current,**options:advance_graph_resident(candidate,dt,current,_local_channel=candidate.calcium_port,**options)
case=read(P/'capture_01/case_00');f=case['inputs'];result={}
...
result['errors']=errors(reference,state);result['speedup']=result['parent']['wall_s']/result['resident']['wall_s'];result['fallbacks']=candidate.backend.fallbacks;result['graph_statistics']=candidate.graph_statistics;result['scope']='PN electrical resident, release chemistryCPU; prescribed1ms, not organism';(out/'RESULT.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2))

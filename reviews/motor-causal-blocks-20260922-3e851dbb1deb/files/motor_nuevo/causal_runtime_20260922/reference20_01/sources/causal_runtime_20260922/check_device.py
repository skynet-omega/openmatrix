...
def run(ids,fail=False):
 ...
 return {int(i):{k:v[n].copy() for k,v in state.items()} for n,i in enumerate(ids)},ledger,e.counts.get()

ids=[0,500,1000,1556];base,events,counts=run(ids);permuted,_,_=run(ids[::-1]);single,_,_=run([500]);retry,_,_=run(ids,True)

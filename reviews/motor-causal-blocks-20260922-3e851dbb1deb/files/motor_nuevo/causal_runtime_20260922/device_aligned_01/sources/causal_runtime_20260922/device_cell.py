...
def source():
    ...
    physical=(HERE.parent/'native_hybrid_20260922/physical_events.cu').read_text()
...
class DeviceCell(Cell):
 def __init__(self,b,events,compressed=False):
  ...
  with cp.cuda.using_allocator(self.pool.malloc),self.stream:
   ...
   self.et=cp.zeros((b.n,self.width));self.ej=cp.zeros_like(self.et);self.ec=cp.zeros(b.n,dtype=cp.int32);self.flag=cp.zeros(b.n,dtype=cp.int32)
 ...
 def advance(self,b,ns,ge,gi,*,current_pA=None,inner_step_ns=25000):
  ...
  with self.stream:
   ...
   args=(np.int32(b.n),np.int64(ns),np.int64(inner_step_ns),np.float64(b.rest),s['delta'],s['gates'],self.ge,self.gi,self.current,b.C,b.G,b.chanG,b.chanb,b.shuntG,b.shuntb,b.ena,*self.vscratch,*self.gscratch,self.errors,self.clock,self.observation,b.caps,b.tau,s['q'],s['last_siz'],s['previous_slope'],s['trough'],s['counts'],s['clipped'],self.ec,self.et,self.ej,self.flag,self.coords,np.float64(self.ts),self.gain,a['q'],a['s'],a['last_voltage'],a['previous_slope'],a['trough'],a['counts'],a['clipped'],self.counts,self.maximum_error,self.status)
  ...
  if len(rr):
   ...
   self.events.active.add(et[rr,cc]+(origin-self.events.start_elapsed)*1e-9,self.events.gamma[rr],ej[rr,cc])

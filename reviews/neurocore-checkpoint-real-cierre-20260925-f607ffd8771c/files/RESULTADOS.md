...
ambas comparaciones completas de los cuatro componentes. Seis alteraciones deliberadas (driver ausente o modificado, manifiesto del
modelo, hash de fuente, parámetro obligatorio ausente y tipo de driver
desconocido) fueron rechazadas antes de ejecutar el cargador. Un séptimo
control comprobó que se cierra un objeto ya cargado si falla la salida del
contexto restaurador. `check_checkpoint_guards.py` produjo el mismo SHA256
normal y con `-O` (`CHECKPOINT_GUARDS.json`). Por ello, **el defecto
...
Se ensayó un segundo contraste acotado: reconstruir también el `Backend` PN,
`CyclicPlan` y `StageGraph` en la rama viva. `run_11_pn_backend_cache` preservó
`core` y `prosthesis` al límite y terminó exactamente como la rama viva en
estado, salida y eventos. La reconstrucción de ese conjunto PN tampoco basta
para reproducir la divergencia del reinicio. Esto no demuestra que otros
propietarios reconstruidos sean inocuos.

Una comprobación separada cargó el checkpoint versionado y comparó los estados
comprometidos antes y después de instalar `RuntimeSession`/motor CUDA, **sin
avanzar**. `after_install_01/RESULT.json` registró igualdad exacta de `core`,
`prosthesis`, reloj, paso propuesto, CNS y sensores pendientes frente al
checkpoint. Una segunda carga sin avanzar (`after_install_02/RESULT.json`)
separó `real_model.install()` de `RuntimeSession(...)`, incluyó tasas, RNG y
metadatos del campo y verificó dos lecturas previas iguales; ambas instalaciones
conservaron todos esos campos. Esto no descarta cachés privadas o trabajo
diferido al primer avance.
...
Las igualdades de estados aquí usan `equal_state`: claves, tipos, formas y
valores numéricos exactos, admitiendo NaN correspondiente. No equivalen a
igualdad byte a byte del archivo ZIP/JSON ni distinguen ceros con signo.
`BYTEWISE_FILE_COMPARISON.json` muestra que los archivos `core` difieren por
orden de claves y numeración de arrays del serializador aunque los estados
decodificados sean iguales. La divergencia de la continuación sí contiene
valores numéricos distintos.

[ChatGPT C++/CUDA](https://chatgpt.com/c/6ab66613-ea8c-83e9-b7eb-7972510f8729)
leyó las fuentes publicadas y revisó el puente; no ejecutó el organismo ni CUDA.
Se incorporaron sus dos observaciones verificables: el hook temporal sólo
restaura en el hilo dueño y un fallo posterior a la carga cierra el modelo.
Además, el driver se decodifica de los mismos bytes cuyo hash se comprobó y
se reutiliza para la comprobación final. La prueba de cierre está entre las
siete guardas CPU; la carga real posterior sin avance pasó. El contrato supone
que el directorio del checkpoint permanece inmutable durante la carga.

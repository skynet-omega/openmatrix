...
def rebuild_pn_stage_graph(obj, rebuild_backend=False):
    """Diagnostic only: rebuild named PN execution data, not committed state."""
    import cupy as cp
    pn = obj.core.hybrid._online_source.pn
    closure = dict(zip(pn.advance.__code__.co_freevars,
                       (cell.cell_contents for cell in pn.advance.__closure__)))
    view = closure.get('view')
    if view is None or not hasattr(view, '_stage_graph') or not hasattr(view, '_stage_graph_key'):
        raise ValueError('PN StageGraph was not active at the 1 ms boundary')
    cp.cuda.runtime.deviceSynchronize()
    before_core = copy.deepcopy(obj.core.state_dict())
    before_prosthesis = copy.deepcopy(obj.state())
    old_key = view._stage_graph_key
    if rebuild_backend:
        from resident import Backend
        view.backend = Backend(pn.backend)
    del view._stage_graph, view._stage_graph_key
    cp.cuda.runtime.deviceSynchronize()
    core_difference = equal_state(obj.core.state_dict(), before_core, 'core')
    prosthesis_difference = equal_state(obj.state(), before_prosthesis, 'prosthesis')
    if core_difference or prosthesis_difference:
        raise ValueError('Cache reconstruction changed committed state: ' +
                         str((core_difference, prosthesis_difference)))
    return {'cache': 'PN Backend+CyclicPlan+StageGraph' if rebuild_backend else 'PN StageGraph',
            'cache_owner': 'pn_execution.install.view',
            'cache_key_sha256': hashlib.sha256(repr(old_key).encode()).hexdigest(),
            'committed_core_equal': True, 'committed_prosthesis_equal': True}


def prepare(out, reference_continuous=None, rebuild_stage_graph=False,
            versioned_driver=False, rebuild_backend=False):
    import cupy as cp
    ...
    try:
        ...
        if reference_continuous is None:
            t = time.perf_counter()
            checkpoint = out / ('checkpoint_v1' if versioned_driver else 'checkpoint')
            if versioned_driver:
                from static_lateral_checkpoint import save as save_with_driver
                save_with_driver(obj, checkpoint)
            else:
                obj.save(checkpoint)
            result['checkpoint_wall_s'] = time.perf_counter() - t
            result['checkpoint_bytes'] = sum(p.stat().st_size for p in
                                             checkpoint.rglob('*') if p.is_file())
            result['checkpoint_versioned_driver'] = versioned_driver
        ...
        if rebuild_stage_graph or rebuild_backend:
            result['cache_discriminator'] = rebuild_pn_stage_graph(obj, rebuild_backend)
...
def reload(out, recover_driver, versioned_driver=False):
    result = {'phase': 'reload', 'status': 'STARTED',
              'test_only_external_driver_recovery': recover_driver,
              'versioned_external_driver': versioned_driver}
    obj = session = undo = None
    try:
        ...
        if versioned_driver:
            if recover_driver:
                raise ValueError('Select only one driver restoration mechanism')
            from static_lateral_checkpoint import load as load_with_driver
            obj = load_with_driver(out / 'checkpoint_v1')
        elif recover_driver:
        ...
        result['prestep_core_difference'] = equal_state(
            obj.core.state_dict(), read_state(out / ('checkpoint_v1/organism/core_carrier/session'
                                                 if versioned_driver else 'checkpoint/core_carrier/session')),
            'prestep_core')
        result['prestep_prosthesis_difference'] = equal_state(
            obj.state(), read_state(out / ('checkpoint_v1/organism/prosthesis'
                                          if versioned_driver else 'checkpoint/prosthesis')),
            'prestep_prosthesis')
        ...
        if (not all(result[k] for k in ('clock_equal', 'state_equal',
                                      ...
                                      'driver_metadata_equal')) or
                result['prestep_core_difference'] is not None or
                result['prestep_prosthesis_difference'] is not None):
            result['status'] = 'INCOMPLETE_RESTORE'
        elif recover_driver or versioned_driver:
            import cupy as cp
            from runtime_session import RuntimeSession
...
def main():
    ...
    parser.add_argument('--rebuild-pn-stage-graph', action='store_true')
    parser.add_argument('--rebuild-pn-backend', action='store_true')
    parser.add_argument('--versioned-driver', action='store_true')
    args = parser.parse_args()
    if args.rebuild_pn_stage_graph and args.rebuild_pn_backend:
        parser.error('Choose one PN cache discriminator')
    ...
    try:
        passed = (prepare(args.out, args.reference_continuous, args.rebuild_pn_stage_graph,
                          args.versioned_driver, args.rebuild_pn_backend)
                  if args.phase == 'prepare' else
                  reload(args.out, args.test_recover_driver, args.versioned_driver))
    finally:
        signal.alarm(0)

...
def load(path):
    ...
    def load_model(model_path):
        obj = AntennalContactRuntime.load(model_path)
        loaded['obj'] = obj
        return obj

    def restore_verified(driver):
        loaded['driver'] = driver
        return _restore_before_validation(driver)

    try:
        obj = external_checkpoint.load(path, load_model, restore_verified)
        state, meta, _, _ = _validate(loaded['driver'])

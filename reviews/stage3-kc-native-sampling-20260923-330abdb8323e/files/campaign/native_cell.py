...
CONTROL=Path('/home/daroch/AXIOMA_ASTRA/motor_nuevo/native_hybrid_20260922/libcell_control.so')

def build_step(groups=None,certificate=None):
 ...
 if groups is not None:
  ...
  code=compile_warp(code,groups,certificate=certificate)
...
class Cell:
 def __init__(self,b,events,compressed=False):
  ...
  self.lib=ct.CDLL(str(CONTROL));l=self.lib;P=ct.c_void_p
  ...
  with cp.cuda.using_allocator(self.pool.malloc),self.stream:
   ...
   self.et=cp.zeros((b.n,self.width));self.ej=cp.zeros_like(self.et);self.ep=cp.zeros_like(self.et);self.ec=cp.zeros(b.n,dtype=cp.int32)
   self.observation=cp.ascontiguousarray(b.obs[1]);self.coords=cp.arange(5,17,dtype=cp.int32)
   groups=None;certificate=None;self.chanG=b.chanG;self.chanb=b.chanb
   if compressed:
    from basis_compile import compile_basis,residual_certificate
    cg,cb,groups,basis_report=compile_basis(cp.asnumpy(b.chanG).reshape(51,17,17),cp.asnumpy(b.chanb).reshape(51,17))
    certificate=residual_certificate(cp.asnumpy(b.chanG).reshape(51,17,17),cp.asnumpy(b.chanb).reshape(51,17),cg,cb,groups,cp.asnumpy(b.ena))
    self.chanG=cp.asarray(cg);self.chanb=cp.asarray(cb);self.report.update(compressed=True,basis=basis_report,original_operator_residual_bound=True)
   self.step_kernel=build_step(groups,certificate);mod=cp.RawModule(code=__import__('kc_trace').physical_source(),options=('--fmad=false',));self.somatic=mod.get_function('commit_event');self.axonal=mod.get_function('commit_axon')
   from kc_trace import Trace
   self.trace=Trace(self,mod)
 ...
 def commit(self):
  ...
  for split,v in ((1,self.va),(2,self.vb)):
   self.somatic(((b.n+255)//256,),(256,),(np.int32(b.n),np.int32(b.ports),np.int32(self.width),np.int32(split),self.clock,v,np.float64(b.rest),self.observation,np.float64(-40.),np.float64(20.),b.caps,b.tau,s['q'],s['last_siz'],s['previous_slope'],s['trough'],s['counts'],s['clipped'],self.ec,self.et,self.ej,self.ep,self.flag))
 ...
 def load_epoch(self,p):
  ...
  cp.copyto(self.gain,p.gain);self.ec.fill(0);self.et.fill(0);self.ej.fill(0);self.ep.fill(0);self.flag.fill(0)
 def advance(self,b,ns,ge,gi,*,current_pA=None,inner_step_ns=25000):
  ...
  if len(rr):self.events.active.add(et[rr,cc]+(origin-self.events.start_elapsed)*1e-9,self.events.gamma[rr],ej[rr,cc],posts=ep[rr,cc])
...
def install(brain,events,compressed=False,method='midpoint'):
 if method not in ('midpoint','ros'):raise ValueError('Unknown membrane method')
 if method=='ros' and compressed:raise ValueError('Separate numerical prototype required')
 ...
 def advance(self,*args,**kw):
  if holder.core is None:
   cls=Cell
   if method=='ros':
    from ros_native import RosCell
    cls=RosCell
   holder.core=cls(self,events,compressed);holder.report=holder.core.report

...
def source():
    ...
    from kc_trace import physical_source
    physical=physical_source()
    physical=physical.replace('extern "C" __global__ void commit_', '__device__ void commit_')
...
class DeviceCell(Cell):
 def __init__(self,b,events,compressed=False,*,validate_publisher=True):
  ...
  with cp.cuda.using_allocator(self.pool.malloc),self.stream:
   ...
   self.module=cp.RawModule(code=source(),options=('--fmad=false',))
   self.kernel=self.module.get_function('cell_epoch')
   from kc_trace import Trace
   self.trace=Trace(self,self.module)

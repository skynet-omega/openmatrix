...
extern "C" __global__ void commit_event(int n,int dim,int width,int split,const long long*clock,
 ...
 kd_save(i,0,split,clock,voltage,last[i],slope,slope0[i],trough[i],q[i],0.,peak,event,counts[i]);
...
extern "C" __global__ void commit_axon(int n,int dim,int axons,int split,const long long*clock,
 ...
 kd_save(cell,port+1,split,clock,voltage,last[j],slope,slope0[j],trough[j],q[j],s[j],peak,event,counts[j]);

"""Verify salvaged raw pstats against paired complete scientific states.

The instrumented runner failed after writing raw pstats; this verifier does
not resume or rerun the organism. It reconstructs caller arcs from that file.
"""
...
CONTROL = ROUND / 'noncns_control_01'
PROFILE = ROUND / 'noncns_profile_01'
...
def main():
    ...
    if (status['profile']['status'] != 'INCOMPLETE' or
            ...
            err.get('type') != 'ModuleNotFoundError' or
            err.get('message') != "No module named 'NONCNS_pstats_arcs'"):
    ...
    arcs = extractor.extract(str(raw))['rows']
    gate = arc_gate(arcs)
    if not gate:
    ...
    targets = {r['kind']: r for r in arcs
               if r['kind'] in ('cupy.ndarray.get', 'numpy.ndarray.tolist')}
    result = {
        'schema': 'noncns_saved_raw_pstats_verification_v1',
        'classification': 'SALVAGED_DIAGNOSTIC_FROM_INCOMPLETE_RUNNER',
    ...
    out = ROUND / 'NONCNS_SALVAGED_VERIFY_01.json'

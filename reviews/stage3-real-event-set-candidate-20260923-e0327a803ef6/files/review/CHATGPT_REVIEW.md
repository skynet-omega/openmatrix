**Reproduje en CPU un mecanismo capaz de producir exactamente `1.0000000000000002` con `lif_record` y `Waveform` publicados. No demuestra todavía que sea la causa del fallo real de ID41645.** La evidencia pendiente es pequeña: los operandos y eventos del puerto en el bloque fallido, más el resultado del control largo sin tap.

## 1. Tres explicaciones rivales

| Hipótesis | Qué la distingue | Prueba corta |
|---|---|---|
| **H1. Pérdida de la transición al exportarla como ADD** | El emisor produjo un valor posterior válido —por ejemplo, exactamente 1—, pero el salto almacenado ya describe una trayectoria superior a 1 incluso en alta precisión. | Recalcular el puerto desde **los FP64 guardados exactamente**, conservando además el valor posterior y la operación del emisor. |
| **H2. Redondeo al evaluar una historia válida** | La trayectoria definida por los operandos permanece ≤1 en alta precisión, pero su evaluación FP64 excede el dominio. | Mismos operandos y orden, CPU/CUDA/alta precisión; comparar cada término. No basta recalcular desde decimales redondeados del informe. |
| **H3. Historia o versión consumida distinta** | Cambian estado inicial, marca, salto, índice o buffer entre productor y proyector, o entre tap on/off. | Comparar las huellas y valores del bloque antes de proyectar. Si son iguales pero cambia el resultado, investigar aritmética/ejecución; si difieren, localizar la primera frontera divergente. |

**H1 tiene un mecanismo concreto en estas fuentes:** `lif_record` decae hasta el final, retrocede para calcular `before`, aplica la saturación física y exporta solamente `event_value-before`. `event_ports.py` reconstruye después la historia mediante una suma de exponenciales. **Los archivos examinados no transportan SET/post**, aunque esa reparación existiera en otra rama.  

**Esto no es evidencia de rigidez.** Para una coordenada prescrita por el proyector, reducir el paso no necesariamente cambia el valor reconstruido en la misma marca. Ambos aproximantes pueden compartir exactamente ese valor inválido. La rigidez exigiría evidencia adicional sobre la integración del sistema acoplado, no simplemente un exceso de 1 ulp. Las bibliotecas matemáticas CPU/GPU tampoco tienen por qué producir resultados idénticos para todas las funciones trascendentes.  :chatgpt-content-reference{index="3"}

El on/off exacto a 1 ms respalda el tap **en ese prefijo**, no su inocuidad a 119 ms. El fallo actual ocurrió con reloj local `[0, 4.486788730489399e-6]`; el recibo no conserva los operandos necesarios para clasificarlo entre H1–H3.  

## 2. Resultado ejecutado y corrección condicionada

Usé un **evento sintético**, no la neurona real:

- \(q_0=0,7\), \(\tau_q=0,024\) s.
- Evento a \(31,25\,\mu s\), dentro de un intervalo de \(125\,\mu s\).
- El emisor publicado produce una transición saturada con valor posterior **1**.
- Su salto exportado es `0.30091086519344823`.

La reconstrucción ADD devuelve:

```text
Waveform publicada, CPU:       1.0000000000000002
Operandos FP64, 110 decimales: 1.0000000000000001642719713861334044885...
Transición SET al post físico: 1.0
```

**Añadir únicamente metadatos al emisor conservó bit a bit sus estados, tiempos, saltos y conteos en el fixture.** El replay por segmentos coincidió con una referencia independiente `scipy.linalg.expm` en las consultas ensayadas; ADD-only difirió en \(1,11\times10^{-16}\).

La justificación es:

\[
q(t)=q^+e^{-(t-t_e)/\tau_q},\qquad
s(t)=s^-e^{-(t-t_e)/\tau_s}+q^+\Phi(t-t_e).
\]

En el evento, \(s\) permanece continuo y \(q^+\) es **la transición producida por el modelo**. Transportar ese valor evita reconstruirlo como \(q^-+(q^+-q^-)\) usando un \(q^-\) numéricamente distinto.

**No propongo convertir cualquier exceso en SET(1).** SET solo procede cuando el emisor declaró esa transición. Los ADD genuinos siguen siendo ADD; no se cambian dominios. Si se incorpora a producción, deben compartir la operación el registro, su consulta CPU y el proyector CUDA; un SET válido no puede descartarse por salto auxiliar cero.

## 3. Código completo: `auditar_evento.py`

El programa usa vuestra fuente publicada para ejecutar el emisor original. Genera una **copia aislada con metadatos**, sin modificar el archivo ni instalarla en el organismo. También incluye la captura forense previa al rollback.
...
```bash
# Fuente del snapshot e30360e..., no otra versión local:
python auditar_evento.py \
  --waveform /ruta/extraida/source/event_waveform.py \
  --out prueba_ulp_01

# Después de obtener los operandos reales antes del rollback:
python auditar_evento.py \
  --case /ruta/diagnostico/PUERTO_28296.json \
  --out replay_41645_01
```

Requiere NumPy, SciPy y Numba. El segundo comando **no puede recuperar una transición SET perdida**: analiza el registro disponible y marca esa limitación.

## 4. Captura del fallo sin rehabilitar el propietario

En `NativeGraph.advance`, dentro de `code!=0` y **antes** de `cp.copyto(self.x,self.backup)`, llamar a `dump_antes_rollback` con el propietario `FilterPorts` real. En este código, normalmente puede obtenerse de `self.project.__self__`; hay que comprobarlo, no asumirlo si existe otro wrapper.

```python
try:
    dump_antes_rollback(
        self, propietario_filter_ports, directorio_forense_nuevo,
        {
            "duration_s": ns * 1e-9,
            "bodyId": 41645,
            "contexto": "epoch y predictor/confirmado declarados por el llamador"
        }
    )
except Exception as error_captura:
    self.last_failure["capture_error"] = repr(error_captura)
# Continuar SIEMPRE el rollback y propagar el error numérico original.
```

La captura guarda **estado anterior, último estado interno, aproximantes, reloj y eventos empaquetados**. No llama a `state_dict()`, no evalúa coeficientes, no acepta el intento y no publica un checkpoint. Si el propietario exterior falló, sigue inválido. Para confirmar H1 también debe capturarse en el emisor `post` y la operación antes de reducirlos a `jump`.

El cierre publicado ya impide el checkpoint fallido. El `AttributeError('coefficients_gpu')` durante limpieza es posterior al error numérico; **no lo trataría como causa del exceso**.  

## 5. Ejecución y alcance

**Ejecutado:** emisor y consulta CPU de la fuente publicada, copia instrumentada con metadatos, referencia `expm`, orden ADD/SET y replay sin imponer un dominio universal. La ejecución final tomó **2,97 s de pared y 2,96 s de CPU**, aproximadamente **201,4 MiB**. Un primer intento falló por caché de Numba/importación dinámica; quedó conservado y se corrigió únicamente el uso de caché del arnés.

**No ejecutado:** CUDA, captura forense con objetos GPU, control largo on/off ni evento real de ID41645. La alta precisión de 90/110 decimales no es una demostración por intervalos.

**Leídos completos:** README raíz y campaña, `ARCHIVE.json`, `MANIFEST.json`, `event_ports.py`, `event_waveform.py`, `graph_core.py`, `native_tap.py`, `run_kernel.py`, `SMOKE_COMPARE.json` y `full_sham_RESULT.json`. El ZIP remoto no pudo descargarse. Materialicé el texto de `event_waveform.py` y comprobé su blob Git y SHA256 contra el manifiesto: `49986d70…ec75fae`. 

:chatgpt-content-reference{index="10"}[Fuente, resultados, fallo conservado y manifiesto](sandbox:/mnt/data/AXIOMA_EVENT_ULP_CPU_REVIEW.zip) · :chatgpt-content-reference{index="11"}[Python completo](sandbox:/mnt/data/AXIOMA_EVENT_ULP_REVIEW/auditar_evento.py)

**Decisión:** capturar y clasificar el bloque real antes de corregir producción. H1 es ahora un mecanismo reproducido; H2 y H3 siguen abiertas para esta corrida. Ninguna de estas pruebas cambia anatomía, acredita una aceleración o supera etapa 3.

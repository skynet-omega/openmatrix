...
TARGET_IDS = (10176, 10208, 10360, 523769, 10065, 10118)
TARGET_TOL = 1e-9
RATE_TOL = 1e-12
...
class NativeFlowTap:
    def __init__(self, hybrid, metadata, output: Path):
        ...
        if any(len(p) != 1 for p in positions):
            raise ValueError('Expected six unique bilateral PN/DNa02/DNb05 identities')
        self.rows = np.asarray([int(p[0]) for p in positions],dtype=np.int32)
        if [types[i] for i in self.rows] != ['DM1_lPN','DM1_lPN','DNa02','DNa02','DNb05','DNb05']:

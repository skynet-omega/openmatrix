...
def source_files() -> tuple[Path,...]:
    return (HERE/'PLAN.json',HERE/'run_long.py',HERE/'verify_long.py',
...
def main() -> int:
    ...
    try:
        ...
        atomic_json(a.out/'RUN_CONTRACT.json',{
            'schema':'stage4_one_second_trajectory_screen_v1','field':a.field,'engine':a.engine,
            ...
            'source_mode':'The selected mirror source remains world-fixed for 1000 ms; this is an exploratory trajectory screen',
        ...
        atomic_json(a.out/'RESULT.json',{'status':status,'error':error,'cleanup_errors':cleanup,
            ...
            'reason':'Single one-second fixed-source trajectory; only a falsification screen. A matched control is required to claim navigation or feedback.'})

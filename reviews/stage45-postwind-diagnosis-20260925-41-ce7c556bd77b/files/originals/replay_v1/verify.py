...
def calculate(plan,donor,arms,source):
    need(tuple(arms)==tuple(plan['arms']),'Arms/order differ from contract')
    ...
    for name,a in arms.items():
        ...
        need(np.max(np.abs(a['time_s']-(.040+np.arange(1,2001)*.001)))<1e-9,'Physical clock drift')
    ...
    return dict(schema='postwind_mechanical_diagnosis_v1',classification='CONFIRMADO_LOCALMENTE_PENDIENTE_AUDITORIA',
...
def verify(folder,mutate=None):
    ...
    for clock in ('CNS_time_ns','PN_time_ns','body_time_ns'):
        need(np.array_equal(data[clock][trial],(40+np.arange(1,2001))*1000000),'Donor clocks')
    ...
    if mutate=='context':arms['no_wind']['forward_mm_s'][1050]+=.01
    if mutate=='criterion':

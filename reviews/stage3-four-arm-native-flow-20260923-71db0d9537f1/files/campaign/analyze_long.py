"""Read late behavior and direct PN/DN signed inputs from four frozen arms."""
...
WINDOWS=(250,275,300,320,350,400)
IDS=(10176,10208,10360,523769)  # R PN, L PN, R DN, L DN.
...
def main():
    ...
    result={'schema':'stage3_four_arm_late_diagnostic_v1',
            'windows_ms_from_odor_on':list(WINDOWS),'stage3_admission':False,
            'model_confounds':['PN10208 fine output versus PN10176 legacy',
    ...
    for name,(receipt,trace,flow_receipt,flow) in arms.items():
        ...
        result['arms'][name]={'status':receipt['status'],
                             ...
                             'DNa02_native_target_nonzero_samples':int(np.count_nonzero(flow['target'][:,2:])) if flow is not None else None,
                             'windows':windows}

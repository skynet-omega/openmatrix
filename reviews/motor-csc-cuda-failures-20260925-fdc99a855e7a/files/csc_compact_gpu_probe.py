...
COMPACT_CUDA = r'''
extern "C" __global__ void select_sources(
 int n,const double* release,double* cache,const bool* event,double eps,
 int* selected,double* deltas,int* count) {
...
extern "C" __global__ void scatter_selected(
 const long long* ptr,const int* dst,const double* w,
 const int* selected,const double* deltas,const int* count,
 const double* caps,const bool* visual,double scale,
 double* excit,double* inhib) {
 int lane=threadIdx.x%32;
 int warp=(blockIdx.x*blockDim.x+threadIdx.x)/32;
 int warps=(gridDim.x*blockDim.x)/32;
 int end=count[0];
 for(int slot=warp;slot<end;slot+=warps){
  int c=selected[slot];double difference=deltas[c];
  for(long long e=ptr[c]+lane;e<ptr[c+1];e+=32){
   int row=dst[e];double weight=w[e];
   if(visual[row]){
    if(weight>=0.)atomicAdd(excit+row,(weight*scale)*difference);
...
def selftest():
    ...
    module = cp.RawModule(code=CUDA + COMPACT_CUDA,
                          options=('--std=c++11', '--fmad=false'))
    seed, select, scatter = [module.get_function(name) for name in
                             ('csr_seed', 'select_sources', 'scatter_selected')]
    ...
    selected, deltas, count = cp.empty(n,dtype=cp.int32),cp.empty(n),cp.zeros(1,dtype=cp.int32)
    ...
    for state in states[1:]:
        ...
        count.fill(0)
        select((1,),(128,),(np.int32(n),d_state,cache,d_event,
                            np.float64(1e-6),selected,deltas,count))
        scatter((1,),(128,),(dcp,ddst,dcw,selected,deltas,count,
                             d_caps,d_visual,np.float64(1.),a,b))
    ...
    return {'status': 'PASS_SYNTHETIC_COMPACT_CSC', 'CUDA_executed': True}
...
def run(out):
    ...
    root = here.parents[1]
    plan_path = here / 'CSC_COMPACT_GPU_PLAN_37.json'
    ...
    module = cp.RawModule(code=CUDA + COMPACT_CUDA, options=('--std=c++11', '--fmad=false',
    ...
    with stream:
        ...
        selected = cp.empty(n,dtype=cp.int32)
        deltas = cp.empty(n,dtype=cp.float64)
        ...
        select_grid = ((n+255)//256,)
        scatter_grid = (p['scatter_blocks'],)
        def reset():
        ...
        for j in range(1,60):
            count.fill(0)
            select(select_grid,(256,),(np.int32(n),dr[j],cache,devent,
                                       np.float64(p['absolute_release_threshold']),
                                       selected,deltas,count))
            scatter(scatter_grid,(128,),(dcp,ddst,dcw,selected,deltas,count,
                                         dcaps,dvisual,np.float64(p['scale']),a,b))

...
def main():
 ...
 p=argparse.ArgumentParser();p.add_argument('--out',type=Path,required=True);p.add_argument('--ms',type=int,default=5);p.add_argument('--native-cell',action='store_true');p.add_argument('--compressed',action='store_true');p.add_argument('--ros',action='store_true');p.add_argument('--device-cell',action='store_true');p.add_argument('--event-boundaries',action='store_true');a=p.parse_args()
 import cupy as cp
 ...
 try:
  ...
  for base in (PARENT,HERE,HERE.parent/'native_hybrid_20260922'):
   for file in base.iterdir():
    if file.suffix in ('.py','.cpp','.cu','.hpp') or file.name=='PLAN.json':
  ...
  if a.device_cell:
   import device_cell
   cell,undo=device_cell.install(h,events);restores.append(undo)
  elif a.native_cell or a.ros:
  ...
  dump(a.out/'RESULT.json',{'status':status,'error':exception,'ms':len(timings),'requested_ms':a.ms,'wall_total_s':time.perf_counter()-start,'device_cell':a.device_cell,'native_cell':a.native_cell,'compressed':a.compressed,'ros':a.ros,'event_boundaries':a.event_boundaries,'times':timings,'rss_GiB':resource.getrusage(resource.RUSAGE_SELF).ru_maxrss/1024**2,'PN_execution':pn,'native_CNS':None if adapter is None else adapter.report,'events':None if events is None else events.report,'cell':None if cell is None else cell.report,'partition':split})

...
sys.path[:0]=[str(HERE/'vendor') if (HERE/'vendor').exists() else str(OLD/'motor13_20260922'),str(HERE.parents[1]/'campanas/etapa3_motor_nuevo_20260922')]
...
certificate=residual_certificate(G,b,cG,cb,groups,ena) if '--certificate' in sys.argv else None
compiled=cp.RawKernel(compile_warp(WARP,groups,certificate=certificate),'warp_midpoint',options=('--fmad=false',))
...
(HERE/('BASIS_CERTIFIED_CHECKS.json' if certificate is not None else 'BASIS_CHECKS.json')).write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report))

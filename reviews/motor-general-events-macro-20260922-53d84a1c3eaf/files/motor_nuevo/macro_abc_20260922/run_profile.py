...
import numpy as np
R=Path(__file__).resolve().parent;T=R.parent/'transient_localization_20260922';ROOT=R.parents[1];H=R.parent/'pipeline_review_20260922'
...
def main():
 p=argparse.ArgumentParser();p.add_argument('--variant',choices=['guard','fine1562','guard_nominal'],required=True);p.add_argument('--ms',type=int,choices=[1,20],required=True);p.add_argument('--out',type=Path,required=True);a=p.parse_args()
 RunStorage(a.out);obj=session=None;undo_metadata=undo_ports=None;start=time.perf_counter();rows=[];times=[];status='INCOMPLETE';error=None
 try:
  import cupy as cp,pandas as pd
  obj,d,plan,Field,field,ports=load(a.out/'inputs');b=obj.core.hybrid
  # All variants preserve125us coupling and mandatory event boundaries.
  ...
  for root in (R,T,H,T.parent/'causal_runtime_20260922',T.parent/'native_hybrid_20260922',ROOT/'campanas/etapa3_motor_nuevo_20260922'):
  ...
  for i in range(a.ms):
   if time.perf_counter()-start>215:raise TimeoutError('Process budget')
   ...
   if i in (0,4,19):write_state(a.out/f'brain_{i+1:02d}ms',b.state_dict())
   print(json.dumps(record),flush=True)
  if b.time_ns!=origin+a.ms*1000000:raise ValueError('Clock mismatch')
  from profile_operator import profile
  atomic_json(a.out/'PROFILE.json',profile(session))
  ...
  atomic_json(a.out/'RESULT.json',{'status':status,'error':error,'variant':a.variant,'ms':len(rows),'requested_ms':a.ms,'wall_s':time.perf_counter()-start,'times':times,'runtime':None if session is None else session.report(),'stage3_admission':False})

...
class Waveform:
    def __init__(self,q,s,tau_q,tau_s,duration):
        ...
        self.ts=float(tau_s);self.duration=float(duration);self.times=[];self.rows=[];self.jumps=[]
    ...
    def add(self,times,rows,jumps):
        ...
        if (r.dtype.kind not in 'iu' or r.ndim!=1 or r.shape!=t.shape or j.shape!=t.shape or not np.isfinite(t).all() or not np.isfinite(j).all()
            or np.any((t<0)|(t>self.duration+1e-15)) or np.any((r<0)|(r>=len(self.q))) or np.any(j<0)):
            raise ValueError('Invalid effective event jumps')
        keep=j>0;self.times.extend(t[keep]);self.rows.extend(r[keep]);self.jumps.extend(j[keep]);self.device=None
...
def lif_record(voltage,refractory_left,counts,q,vinf,rate,reset,threshold,caps,filter_tau,dt,refractory=.0022):
    ...
    max_events=2+int(dt/refractory);times=np.full((len(q),max_events),np.nan);jumps=np.zeros_like(times);clipped=0
    for k in range(len(voltage)):
        ...
        while t<dt:
            ...
            times[k,n]=t;jumps[k,n]=event_value-before;n+=1

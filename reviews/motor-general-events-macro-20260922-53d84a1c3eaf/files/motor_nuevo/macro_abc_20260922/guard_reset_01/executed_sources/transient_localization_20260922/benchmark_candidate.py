...
def main():
 ...
 RunStorage(a.out);obj=session=None;start=time.perf_counter();rows=[];times=[];status='INCOMPLETE';error=None
 try:
  ...
  session=RuntimeSession(b,'causal_cuda')
  ...
  for root in (T,H,T.parent/'causal_runtime_20260922',T.parent/'native_hybrid_20260922',ROOT/'campanas/etapa3_motor_nuevo_20260922'):
  ...
  for i in range(a.ms):
   if time.perf_counter()-start>190:raise TimeoutError('Process budget')

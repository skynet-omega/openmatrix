...
def advance_graph_resident(self,dt_ns,current_pA,*,rtol=1e-9,atol=2e-12,maxiter=220,
    ...
    def stage(vbase,xbase,guess,synapse,ca_base,ca_update,threshold_guess=None):
        # The speculative graph performs two Newton corrections. A caller's
        # smaller budget must use the original bounded iteration instead.
        if max_newton < 2:
            return fallback_stage(vbase,xbase,guess,synapse,ca_base,ca_update,threshold_guess)

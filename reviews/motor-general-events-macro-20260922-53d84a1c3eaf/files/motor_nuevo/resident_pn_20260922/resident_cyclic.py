...
class CyclicTree:
 def __init__(self,plan,shift):
  ...
  path=Path(__file__).parent.parent/'pn_abc_20260922';code=(path/'parallel_tree.cu').read_text()+(path/'cyclic_tree.cu').read_text()+(path/'parallel_core.cu').read_text();module=cp.RawModule(code=code,options=('--fmad=false',));forward=module.get_function('messages');gather=module.get_function('gather');back=module.get_function('back_level');core=module.get_function('core_parallel')

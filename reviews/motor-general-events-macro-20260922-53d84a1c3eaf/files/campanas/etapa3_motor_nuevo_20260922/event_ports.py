...
extern "C" __global__ void port(double*y,const long long*qr,const long long*sr,const double*q,const double*s,const double*tq,const double*ts,const double*et,const double*ej,const int*counts,int n,int width,const double*clock,double fraction){
...
class FilterPorts:
 def __init__(self,q_rows,s_rows,width=8):
  ...
  self.times=cp.zeros((self.n,width));self.jumps=cp.zeros_like(self.times);self.counts=cp.zeros(self.n,dtype=cp.int32)
 ...
 def update(self,w):
  ...
  t=np.zeros((self.n,self.width));j=np.zeros_like(t);used=np.zeros(self.n,dtype=int)
  for tt,rr,jj in zip(w.times,w.rows,w.jumps):t[rr,used[rr]]=tt;j[rr,used[rr]]=jj;used[rr]+=1
  for dst,src in ((self.q,w.q),(self.s,w.s),(self.tq,w.tau),(self.ts,np.array([w.ts])),(self.times,t),(self.jumps,j),(self.counts,count)):
   dst.set(np.ascontiguousarray(src))
 def project(self,y,clock,fraction):
  out=y.copy()
  self.kernel(((self.n+255)//256,),(256,),(out,self.qr,self.sr,self.q,self.s,self.tq,self.ts,self.times,self.jumps,self.counts,np.int32(self.n),np.int32(self.width),clock,np.float64(fraction)))
  return out

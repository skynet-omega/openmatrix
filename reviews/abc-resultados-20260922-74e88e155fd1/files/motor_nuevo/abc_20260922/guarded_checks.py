...
CHECK='''extern "C" __global__ void inspect(int n,const double* x,unsigned int* flags,bool nonnegative){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i<n){if(!isfinite(x[i]))atomicOr(flags,1U);if(fabs(x[i])>1.e12)atomicOr(flags,4U);if(nonnegative&&x[i]<0.)atomicOr(flags,2U);}}'''
...
def main():
 ...
 for condition in c['conditions']:
  for spec in specs:
   ...
   if flag&5:raise ValueError(f'Nonfinite or excessive intermediate state {name}, flags={flag}')
   ...
   row={'run':name,'persistent_flags':flag,'supported_C_sign_domain':not bool(flag&2),'max_trajectory_change':error,'wall_s':r['wall_with_sampling_s']}

...
extern "C" __global__ void norm(const double*a,const double*b,int n,int normn,double atol,double rtol,double*err,int*flag,int*domain,const double*lower,const double*upper){__shared__ double z[256];int i=blockIdx.x*blockDim.x+threadIdx.x;double e=0;if(i<n){if(!isfinite(a[i])||!isfinite(b[i]))atomicOr(flag,1);if(b[i]<lower[i]||b[i]>upper[i])atomicOr(domain,1);if(i<normn)e=fabs(a[i]-b[i])/(atol+rtol*fmax(fabs(a[i]),fabs(b[i])));if(!isfinite(e)){atomicOr(flag,1);e=0;}}z[threadIdx.x]=e;__syncthreads();for(int k=128;k;k/=2){if(threadIdx.x<k)z[threadIdx.x]=fmax(z[threadIdx.x],z[threadIdx.x+k]);__syncthreads();}if(threadIdx.x==0)atomicMax((unsigned long long*)err,__double_as_longlong(z[0]));}
...
class NativeGraph:
 ...
 def __init__(self,initial,coefficient,*,rtol,atol,norm_size,project=None,freeze=None,native_library=None,state_bounds=(0.,1.)):
  if native_library is None:raise ValueError("B requiere biblioteca padre explicita")
 ...
 def advance(self,ns,next_ns,min_ns,max_ns,budget=30,boundaries=None):
  if boundaries is None:raise ValueError("B requiere agenda explicita; [] si no hay eventos")
